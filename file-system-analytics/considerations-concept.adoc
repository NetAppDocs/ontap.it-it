...
Se si verifica un notevole aumento della latenza, fare riferimento alink:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/High_or_fluctuating_latency_after_turning_on_NetApp_ONTAP_File_System_Analytics["Base di conoscenza NetApp : Latenza elevata o fluttuante dopo l'attivazione di NetApp ONTAP File System Analytics"^] .



== Considerazioni sulla scansione

Quando abiliti l'analisi della capacità, ONTAP esegue una scansione di inizializzazione per l'analisi della capacità. La scansione accede ai metadati di tutti i file nei volumi per cui l'analisi della capacità è abilitata. Durante la scansione non vengono letti i dati dei file. A partire da ONTAP 9.14.1, puoi monitorare l'avanzamento della scansione tramite l'ONTAP REST API, nella scheda **Explorer** di System Manager o con il comando CLI di ONTAP  `volume analytics show`. Se si verifica un evento di limitazione, ONTAP fornisce una notifica.
