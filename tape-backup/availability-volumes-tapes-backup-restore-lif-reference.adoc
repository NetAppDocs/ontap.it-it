...
summary: 'È possibile configurare un"applicazione per il backup per stabilire una connessione di controllo NDMP su qualsiasi tipo di LIF in un cluster. Nella modalità NDMP a livello di macchina virtuale di storage, è possibile determinare la disponibilità di volumi e dispositivi a nastro per le operazioni di backup e ripristino in base a questi tipi di LIF e allo stato dell"estensione CAB.' 
---
= Disponibilità di volumi ONTAP e dispositivi a nastro per il backup e il ripristino su diversi tipi di LIF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
