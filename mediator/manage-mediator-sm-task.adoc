...
Configurare il servizio Mediator
 a| 
Eseguire le operazioni descritte in link:../task_metrocluster_configure.html#configure-the-ontap-mediator-service["Configurare il servizio ONTAP Mediator"].
...
Controllare lo stato del mediatore
 a| 
Eseguire le operazioni descritte in link:../task_metrocluster_troubleshooting.html["Risoluzione dei problemi relativi alle configurazioni di IP MetroCluster"].
...
Eseguire uno switchover e uno switchback
 a| 
Eseguire le operazioni descritte in link:../task_metrocluster_switchover_switchback.html["Eseguire lo switchover e lo switchback di IP MetroCluster"].
