...
* In ONTAP Mediator, il pacchetto SCST viene utilizzato per fornire iSCSI per la comunicazione nelle configurazioni IP MetroCluster. Il pacchetto SCST è un modulo del kernel che viene compilato durante l'installazione. A partire da ONTAP 9.19.1 e ONTAP Mediator 1.12, puoi configurare un ONTAP Mediator che utilizza HTTPS invece di iSCSI per monitorare una configurazione IP MetroCluster.
...
[NOTE]
====
* La porta iSCSI è necessaria solo in una configurazione IP MetroCluster.
* La porta 22/tcp non è necessaria per il normale funzionamento, ma è possibile abilitarla temporaneamente per la manutenzione e disabilitarla quando la sessione di manutenzione è terminata.
...
NOTE: Per un SnapMirror active sync o MetroCluster IP ONTAP Mediator che utilizza HTTPS per la comunicazione, ONTAP non richiede che la porta 3260 sia abilitata o connessa.
...
NOTE: Se hai installato ONTAP Mediator 1.12 o versioni successive con supporto solo per HTTPS, il pacchetto SCST non è installato, quindi i requisiti di compatibilità del kernel non si applicano.
...
* Non aggiornare il kernel oltre la versione del sistema operativo supportata per la tua release di ONTAP Mediator, poiché il modulo SCST testato potrebbe non funzionare.
