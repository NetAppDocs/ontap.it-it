...
Esaminate questi punti prima di aggiornare o installare ONTAP Mediator.
...
.Requisiti di rete
Assicurarsi che le seguenti porte siano disponibili e non utilizzate per ONTAP Mediator:
...
|===
Per i clienti SMBC, ONTAP non richiede che la porta 3260 sia abilitata o connessa.

* Se si utilizza un firewall di terze parti, vedere link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["Requisiti del firewall per ONTAP Mediator"^].
...
* Non aggiornare il kernel oltre la versione del sistema operativo supportata per la tua release di ONTAP Mediator, poiché il modulo SCST testato probabilmente non funzionerà.
