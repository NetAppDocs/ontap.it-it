...
ONTAP Mediator si basa su SCST per fornire iSCSI (vedere  http://scst.sourceforge.net/index.html[]). Questo pacchetto è un modulo del kernel che viene compilato durante l'installazione specificamente per il kernel. Qualsiasi aggiornamento del kernel potrebbe richiedere la reinstallazione di SCST. In alternativa, disinstallare e quindi reinstallare ONTAP Mediator, quindi riconfigurare la relazione ONTAP.


NOTE: Qualsiasi aggiornamento al kernel del sistema operativo del server deve essere coordinato con una finestra di manutenzione in ONTAP.
