...
summary: 'Per installare il servizio di supporto ONTAP, è necessario ottenere il pacchetto di installazione ed eseguire il programma di installazione sull"host.' 
---
...
NOTE: Per saltare il controllo automatico della firma durante l'installazione, utilizzare il seguente comando: `./ontap-mediator-1.9.0/ontap-mediator-1.9.0 -y --skip-code-signature-check`
...
. A partire da ONTAP Mediator 1.4, il meccanismo di avvio sicuro è abilitato sui sistemi UEFI. Quando Secure Boot è attivato, è necessario eseguire ulteriori operazioni per registrare la chiave di sicurezza dopo l'installazione:
+
...
NOTE: Dopo l'installazione, i file README e la posizione della chiave vengono forniti anche nell'output di sistema.
