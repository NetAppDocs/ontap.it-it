...
[CAUTION]
====
* Nelle configurazioni IP di MetroCluster, quando ONTAP Mediator è configurato con il protocollo HTTPS, una singola istanza di ONTAP Mediator può monitorare una sola configurazione IP di MetroCluster. Il monitoraggio simultaneo di più configurazioni IP di MetroCluster non è supportato.
...
** Rispondi `No` se prevedi di utilizzare ONTAP Mediator solo per SnapMirror active sync, oppure se prevedi di configurare ONTAP Mediator con HTTPS per monitorare una singola configurazione IP MetroCluster per ogni istanza di ONTAP Mediator.
+
In alternativa, è possibile utilizzare l' `--https-only`opzione nel `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y`comando per specificare solo HTTPS.
...
[NOTE]
====
Per saltare il controllo della firma durante l'installazione, utilizzare questo comando: `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
