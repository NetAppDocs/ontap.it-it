...
summary: 'Per installare o aggiornare ONTAP Mediator, è necessario ottenere il pacchetto di installazione ed eseguire il programma di installazione sull"host.' 
---
...
* Durante l'installazione o l'aggiornamento a ONTAP Mediator 1.12 o versioni successive, il programma di installazione richiede di rispondere alle seguenti:
+
`ONTAP Mediator can be installed with iSCSI and HTTPS support, or HTTPS support only.
Do you wish to install ONTAP Mediator with iSCSI and HTTPS support? Y(es)/n(o):`

+
** Indica  `Yes` se intendi utilizzare ONTAP Mediator configurato con iSCSI per monitorare una o più configurazioni IP MetroCluster.
** Rispondi `No` se hai intenzione di usare ONTAP Mediator solo per SnapMirror active sync, oppure se hai intenzione di configurare ONTAP Mediator con HTTPS per monitorare una o più configurazioni IP MetroCluster.
+
In alternativa, è possibile utilizzare l' `--https-only`opzione nel `./ontap-mediator-1.12.1/ontap-mediator-1.12.1`comando per specificare solo HTTPS.



* Per i casi d'uso di automazione, puoi usare il comando  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y` per saltare le richieste dell'installer.
...
[listing]
----
[root@mediator_host ~]# ./ontap-mediator-1.12.1/ontap-mediator-1.12.1
----
+
[NOTE]
====
Per saltare il controllo della firma durante l'installazione, utilizzare questo comando: `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`

====
+
Il programma di installazione crea gli account richiesti e installa i pacchetti richiesti.  Se Mediator è già installato, ti verrà chiesto di aggiornarlo.



.Esempio di installazione di ONTAP Mediator con supporto HTTPS e iSCSI (output della console)
[%collapsible]
====
[listing]
----

[root@mediator_host ~]# tar -zxvf ontap-mediator-1.12.1.tgz
...
OS package installations finished
    OS package installations finished
    + Installing ONTAP Mediator. (Log: /root/ontap_mediator.pJBcIv/ontap-mediator-1.12.1/ontap-mediator-1.12.1/install_20260324151410.log)
...
----
====
.Esempio di aggiornamento di ONTAP Mediator con supporto HTTPS e iSCSI (output della console)
[%collapsible]
====
[listing]
----
...
ONTAP Mediator will be installed with mode: ISCSI_HTTPS
+ Selected install package mode: ISCSI_HTTPS
ONTAP Mediator will be upgraded from version 1.11.0 to 1.12.1.
...
+ Upgrading ONTAP Mediator. (Log: /root/ontap_mediator.gwaGmU/ontap-mediator-1.12.1/ontap-mediator-1.12.1/upgrade_20260420145942.log)
...
----
====
.Esempio di installazione di ONTAP Mediator con supporto solo per HTTPS (output della console)
[%collapsible]
====
[listing]
----
...
ontap-mediator-1.12.1/ontap-mediator-1.12.1.tsr
ontap-mediator-1.12.1/ontap-mediator-1.12.1.sig.tsr
[root@mediator_host ~]# ontap-mediator-1.12.1/ontap-mediator-1.12.1 --https-only
...
Complete!
OS package installations finished
+ Installing ONTAP Mediator. (Log: /root/ontap_mediator.CJqNqu/ontap-mediator-1.12.1/ontap-mediator-1.12.1/install_20260420120204.log)
    This step will take several minutes. Use the log file to view progress.
    Sudoer config verified
    ONTAP Mediator rsyslog and logging rotation enabled
...
----
====
.Esempio di aggiornamento di ONTAP Mediator con supporto solo per HTTPS (output della console)
[%collapsible]
====
[listing]
----
...
Database upgrade operation complete.
+ Adding python312 specific packages
+ Upgrading ONTAP Mediator. (Log: /root/ontap_mediator.cRy0mN/ontap-mediator-1.12.1/ontap-mediator-1.12.1/upgrade_20260421113003.log)
...
== Registra la chiave di sicurezza per UEFI Secure Boot

A partire da ONTAP Mediator 1.4, il meccanismo Secure Boot è abilitato sui sistemi UEFI.  Se è abilitato l'avvio protetto, è necessario eseguire ulteriori passaggi per registrare la chiave di sicurezza dopo l'installazione.


NOTE: Se hai installato ONTAP Mediator 1.12 o versioni successive con supporto solo per HTTPS, il pacchetto SCST non è installato e puoi saltare questo passaggio.
...
NOTE: Dopo l'installazione, l'output di sistema fornisce i file README e la posizione della chiave.
...
[NOTE]
====
È possibile lasciare la chiave privata nella sua posizione predefinita oppure spostarla in una posizione sicura.  È necessario conservare la chiave pubblica nella sua posizione attuale affinché Boot Manager possa utilizzarla.  Per ulteriori informazioni, consultare il file README.module-signing:

`[root@hostname ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv`

====
. Riavvia l'host e utilizza UEFI Boot Manager del tuo dispositivo per approvare il nuovo MOK.  Avrai bisogno della passphrase fornita per `mokutil` utilità inlink:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["Installare ONTAP Mediator quando l'avvio protetto UEFI è attivato"] .




[[signing-scst-kernel-modules]]
== Firma dei moduli del kernel SCST

Dopo l'installazione ONTAP Mediator, se lo stato systemctl `mediator-scst` viene visualizzato come non riuscito (inattivo), seguire questi passaggi per firmare il modulo kernel SCST.


NOTE: Se hai installato ONTAP Mediator 1.12 o versioni successive con supporto solo per HTTPS, il pacchetto SCST non è installato e puoi saltare questo passaggio.

.Fasi
. Durante il processo di compilazione viene generata una coppia di chiavi pubblica/privata nel `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/` directory, utilizzando il seguente comando:
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. Avviare il processo di importazione della chiave pubblica nel repository delle chiavi UEFI eseguendo i seguenti comandi:
+
...
. Il software mokutil richiede una password temporanea da utilizzare per questa chiave durante il processo di importazione.
. Verificare se il processo di importazione è iniziato con `mokutil --list-new` e quindi riavviare il sistema.  Il bootloader avvia il gestore EFI MOK.
. Utilizzare i menu sullo schermo per attivare il tasto del modulo kernel SCST.  Dopo l'avvio, eseguire `systemctl status mediator-scst` .  Una volta avviato il servizio, i moduli del kernel SCST vengono firmati.
