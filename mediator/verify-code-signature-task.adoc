...
[NOTE]
====
* A partire dal 15 giugno 2025, non sarà più possibile installare o aggiornare ONTAP Mediator alle versioni 1.9 e 1.8 perché i certificati di verifica della firma del codice sono scaduti. Invece, installa o aggiorna ONTAP Mediator alla versione 1.10 o successiva.
...
`ontap-mediator-1.12.0`
 a| 
L'eseguibile di installazione del prodotto per la versione 1.12



 a| 
`ontap-mediator-1.12.0.sig`
 a| 
...
----
. Verifica i  `ontap-mediator-1.12.0.sig.tsr` e  `ontap-mediator-1.12.0.tsr` file utilizzando i certificati associati:
+
[role="tabbed-block"]
====
.OpenSSL 3.x
--
`openssl ts -verify -data ontap-mediator-1.12.0.sig -in ontap-mediator-1.12.0.sig.tsr -CAfile tsa-prod-chain-ONTAP-Mediator.pem -untrusted tsa-prod-ONTAP-Mediator.pem`

--
.OpenSSL 1.x
--
`openssl ts -verify -data ontap-mediator-1.12.0 -in ontap-mediator-1.12.0.tsr -CAfile tsa-prod-chain-ONTAP-Mediator.pem -partial_chain`
...
[listing]
----
openssl dgst -sha256 -verify ONTAP-Mediator-production.pub -signature ontap-mediator-1.12.0.sig ontap-mediator-1.12.0
----
