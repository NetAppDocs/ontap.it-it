...
NOTE: I seguenti file sono inclusi nel pacchetto di download:
...
`ontap-mediator-1.6.0`
 a| 
Il file eseguibile per l'installazione del prodotto per la versione 1.6.0
...
----
. Verificare `ontap-mediator-1.5.0.sig.tsr` e. `ontap-mediator-1.6.0.tsr` file che utilizzano i certificati associati:
+
`openssl ts -verify`
