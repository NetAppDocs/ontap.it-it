...
summary: Gestire ONTAP Mediator, inclusa la gestione delle credenziali utente, il controllo dei servizi, la verifica dello stato, la manutenzione SCST, la gestione dei certificati e la risoluzione dei problemi. 
---
...
+
image:mediator-certs-index.png["Configurazione del server Web e configurazione del mediatore ONTAP"]
...
+
L'autorità PKI verifica la richiesta e firma la `.csr` , generando il certificato `intermediate.crt` .  Inoltre, è necessario ottenere il `root_ca.crt` certificato che ha firmato il `intermediate.crt` certificato dell'autorità PKI.

+

NOTE: Per i cluster SnapMirror Business Continuity (SM-BC) e le configurazioni MetroCluster IP che utilizzano ONTAP Mediator configurato con HTTPS, devi aggiungere i certificati  `intermediate.crt` e  `root_ca.crt` a un cluster ONTAP. Vedi link:../snapmirror-active-sync/mediator-install-task.html["Configurare ONTAP Mediator e cluster per SnapMirror ActiveSync"] e link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["Configura il mediatore ONTAP per una configurazione IP MetroCluster"^].
...
NOTE: Per i cluster SnapMirror Business Continuity (SM-BC), è necessario aggiungere i `ca.crt` certificati e `root_ca.crt` a un cluster ONTAP. Vedere link:../snapmirror-active-sync/mediator-install-task.html["Configurare ONTAP Mediator e cluster per SnapMirror ActiveSync"].
...
. [[step_2_intermediate_info_v9]]Per generare un certificato server dal CSR, eseguire il seguente comando da  `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config` cartella:
+
...
. [[step_2_intermediate_info_v8]]Per generare un certificato server dal CSR, eseguire il seguente comando da  `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config` cartella:
+
...
[role="tabbed-block"]
====
.ONTAP Mediator 1.10 e versioni successive
--
...
. Aggiorna il `/opt/netapp/lib/ontap_mediator/uvicorn/config.json` file.
+
Aggiorna i valori di `ssl_keyfile` , `ssl_certfile` , E `ssl_ca_certs` :
...
+
** IL `ssl_keyfile` il valore è il percorso chiave nel `ontap_mediator_server.crt` file, che è `ontap_mediator_server.key` .
** IL `ssl_certfile` il valore è il percorso del `ontap_mediator_server_chain.crt` file.
** IL `ssl_ca_certs` il valore è il percorso del `root_ca.crt` file.
...
NOTE: Se si è utilizzato `intermediate.crt` per firmare il `ontap_mediator_server.crt` certificato, il  `intermediate.srl` file viene generato. Per ulteriori informazioni, vedere <<step_2_intermediate_info_v9,Passaggio 2: Generare un certificato server firmando con una certificazione CA di terze parti>> .
...
----
+
.. Se si utilizza una struttura di certificato in cui `root_ca.crt` il certificato fornisce un `intermediate.crt` certificato che firma il `ontap_mediator_server.crt` certificato, crea una catena di certificati dal `intermediate.crt` E `ontap_mediator_server.crt` certificati:
+
...
NOTE: Per i cluster SnapMirror Business Continuity (SM-BC), è necessario aggiungere `intermediate.crt` E `root_ca.crt` certificati a un cluster ONTAP . Vederelink:../snapmirror-active-sync/mediator-install-task.html["Configurare ONTAP Mediator e cluster per SnapMirror ActiveSync"] .
...
NOTE: Se si è utilizzato `ca.crt` per firmare il `ontap_mediator_server.crt` certificato, il  `ca.srl` file viene generato. Per ulteriori informazioni, vedere <<step_2_intermediate_info_v8,Passaggio 2: Generare un certificato server firmando con una certificazione CA di terze parti>> .
