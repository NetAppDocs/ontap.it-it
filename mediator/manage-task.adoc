...
NOTE: Per i cluster SnapMirror Business Continuity (SM-BC), è necessario aggiungere i `intermediate.crt` certificati e `root_ca.crt` a un cluster ONTAP. Vedere link:../snapmirror-active-sync/mediator-install-task.html["Configurare ONTAP Mediator e i cluster per la sincronizzazione attiva di SnapMirror"].
