...
NOTE: Se stai eseguendo un'installazione per la prima volta e desideri preinstallare SCST prima di installare ONTAP Mediator, puoi saltare questo passaggio.  Durante l'installazione, il programma di installazione ONTAP Mediator applica tutte le patch rilevanti ai componenti SCST.
...
NOTE: Per i cluster SnapMirror Business Continuity (SM-BC), è necessario aggiungere i `intermediate.crt` certificati e `root_intermediate.crt` a un cluster ONTAP. Vedere link:../snapmirror-active-sync/mediator-install-task.html["Configurare ONTAP Mediator e cluster per SnapMirror ActiveSync"].
