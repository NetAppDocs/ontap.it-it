...
[role="lead"]
Assicurarsi che il sistema operativo host per ONTAP Mediator venga sottoposto a manutenzione regolare per ottenere prestazioni ottimali.
...
Riavviare l'host solo quando i cluster sono integri. I cluster non possono rispondere ai guasti mentre ONTAP Mediator è offline. Impostare un orario per la manutenzione prima del riavvio.
...
== Aggiornare il kernel del sistema operativo host

SCST deve essere compilato per il kernel che stai utilizzando.  Per aggiornare il sistema operativo, è necessario pianificare un periodo di manutenzione.


NOTE: Se hai installato ONTAP Mediator 1.12 o versioni successive con supporto solo per HTTPS, il pacchetto SCST non è installato e il kernel del sistema operativo host può essere aggiornato senza influire su ONTAP Mediator. Consulta la documentazione del sistema operativo host per le istruzioni di aggiornamento.

.Fasi
Per aggiornare il kernel del sistema operativo host, seguire questi passaggi.
...
* Eseguire questo passaggio prima di installare ONTAP Mediator.  Se la versione SCST in uso è più recente della versione fornita con il programma di installazione ONTAP Mediator, il programma di installazione salta questo passaggio.
* In alcuni scenari, l'installazione predefinita di SCST sul sistema operativo aggiornato può non riuscire perché il sistema operativo aggiornato richiede una patch.
+
A partire da ONTAP Mediator 1.12.1, se l'installazione non riesce, puoi usare lo strumento di riparazione SCST che si trova in  `/usr/local/bin/mediator_repair_scst` per riparare l'installazione.

+
Se il sistema operativo aggiornato richiede una patch, il tool la contiene per la versione di SCST fornita all'interno del pacchetto ONTAP Mediator. Il  `mediator_repair_scst` tool è un tool standalone separato dal programma di installazione principale ed è incluso all'interno del file ONTAP Mediator  `tgz`. Non influisce sull'installazione o sui percorsi di aggiornamento di ONTAP Mediator.
...
* Eseguire questa operazione solo se i certificati autofirmati non sono più validi perché il nome host o l'indirizzo IP cambiano dopo l'installazione ONTAP Mediator.
