...
---
= Nuove funzionalità di ONTAP Mediator
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Supporto per RHEL:
+
** Compatibile: 10.1 e 9.7.
** Consigliati: 10.2, 10.0, 9.8, 9.6, 9.4 e 8.10.


* Supporto per Rocky Linux 10.2, 9.8 e 8.10.
* Supporto per Oracle Linux 10.1 e 9.8 con kernel rhck e uek.
* A partire da ONTAP Mediator 1.12.1 e ONTAP 9.19.1, una singola istanza di ONTAP Mediator configurata con HTTPS può monitorare simultaneamente le seguenti configurazioni:
+
** Un massimo di dieci configurazioni IP MetroCluster a quattro nodi o cinque a otto nodi se installate su un host con una CPU a 8 core e almeno 16GB di RAM. Vedi link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip["Utilizza ONTAP Mediator per monitorare più configurazioni IP MetroCluster"^].
** Una configurazione IP MetroCluster a quattro nodi e una relazione peer di sincronizzazione attiva SnapMirror se installato su un host con una CPU a 2 core. Vedi link:../snapmirror-active-sync/mediator-install-task.html#ontap-mediator["Configurare ONTAP Mediator per SnapMirror ActiveSync"].


* Include un nuovo tool per riparare un'installazione SCST danneggiata. Dopo aver installato ONTAP Mediator, puoi trovare il tool su  `/usr/local/bin/mediator_repair_scst`. Puoi usare il tool dopo un aggiornamento del sistema operativo, se il sistema operativo aggiornato richiede una patch SCST che non è disponibile nel pacchetto SCST predefinito.
...
* Supporto per Rocky Linux 10.2, 9.8 e 8.10.
* Supporto per Oracle Linux 10.1 e 9.7 con kernel rhck e uek.
* A partire da ONTAP 9.19.1 e ONTAP Mediator 1.12, puoi configurare un ONTAP Mediator che utilizza HTTPS per la comunicazione nelle configurazioni IP di MetroCluster. HTTPS offre una maggiore sicurezza crittografando l'intera sessione tramite SSL/TLS. Puoi scegliere di installare ONTAP Mediator con il solo supporto per HTTPS. Il pacchetto SCST non viene installato se utilizzi questo metodo di installazione. link:index.html#https-iscsi-installation["Scopri di più sulle opzioni di installazione HTTPS e iSCSI per ONTAP Mediator"].
...
1,11
 a| 
* Supporto per RHEL 10.1, 10.0, 9.7, 9.6, 9.4 e 8.10.
* Supporto per Rocky Linux 10.1, 9.7 e 8,10.
* Supporto per Oracle Linux 10.0 e 9.6 con kernel rhck e uek.
* Aggiunge il supporto per IPv6 per le configurazioni IP MetroCluster .
* Aggiunge il supporto per fapolicyd.
...
1,10
 a| 
* Supporto per RHEL 10.0, 9.6, 9.4 e 8.10.
* Supporto per Rocky Linux 10.0, 9.6 e 8,10.
* Aggiorna la versione base di Python da Python 3.9 a Python 3.12.
...
* Aggiunge nuovi certificati per la verifica della firma del codice.
* Aggiunto supporto per saltare i controlli della firma del codice utilizzando  `-skip-code-signature-check` bandiera.
* Il programma di installazione visualizza avvisi quando rileva certificati di firma del codice scaduti.
...
* È stata aggiunta una firma crittografica al programma di installazione.
...
| So per mediatore ONTAP | 1.12.1 | 1,12 | 1,11 | 1,10 | 1.9.1 | 1,9 | 1,8 | 1,7 | 1.6 | 1.5 | 1.4 | 1.3 | 1.2 | 1.1 | 1.0 
...
| Mediatore ONTAP 1.11 | scst-3.9.tar.gz 
