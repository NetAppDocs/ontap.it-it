...
* Quando avvii la creazione dello snapshot, puoi impostare un valore di timeout opzionale compreso tra 5 e 120 secondi. Se non imposti un valore di timeout, l'operazione va in timeout con il valore predefinito di 7 secondi. Nell'ONTAP REST API, imposta il valore di timeout con il parametro  `action_timeout`. Nell'ONTAP CLI, usa il flag  `-timeout`.


.Fasi
Puoi completare uno snapshot a due fasi con l'ONTAP REST API o, a partire da ONTAP 9.14.1, con l'ONTAP CLI. Questa operazione non è supportata in System Manager.


NOTE: Se avvii la creazione dello snapshot tramite l'ONTAP REST API, devi confermare lo snapshot tramite l'ONTAP REST API. Se avvii la creazione dello snapshot tramite l'ONTAP CLI, devi confermare lo snapshot tramite l'ONTAP CLI. La combinazione dei metodi non è supportata.
