...
Puoi visualizzare i dati di monitoraggio nella scheda **Overview** del menu del consistency group in System Manager oppure richiedendoli tramite l'ONTAP REST API. A partire da ONTAP 9.14.1, puoi visualizzare le metriche del consistency group con la ONTAP CLI usando il comando  `consistency-group metrics show`. Scopri di più su  `consistency-group metrics show` nella link:https://docs.netapp.com/us-en/ontap-cli/vserver-consistency-group-metrics-show.html["Riferimento al comando ONTAP"^].


NOTE: In ONTAP 9.13.1, puoi recuperare le metriche storiche solo utilizzando l'ONTAP REST API. A partire da ONTAP 9.14.1, le metriche storiche sono disponibili anche in System Manager.
...
I gruppi di coerenza offrono protezione coerente con l'applicazione, garantendo la coerenza dei tuoi dati su più volumi o LIF. Quando crei uno snapshot di un gruppo di coerenza, viene stabilita una "recinzione" sul gruppo di coerenza. La recinzione avvia una coda per l'I/O fino al completamento dell'operazione di snapshot, garantendo la coerenza point-in-time dei dati su tutte le entità del gruppo di coerenza. La recinzione può causare un picco transitorio di latenza durante la creazione dello snapshot, ad esempio con una policy di snapshot pianificata o creando uno snapshot con System Manager. Scopri di più sul contesto dell'ONTAP REST API e dell'ONTAP CLI nella link:https://kb.netapp.com/on-prem/ontap/DM/REST-API["Documentazione dell'API REST di ONTAP"^] e nella link:https://docs.netapp.com/us-en/ontap-cli/["Riferimento al comando ONTAP"^].
...
I volumi e gli snapshot associati ai gruppi di coerenza posizionati su aggregati con mirroring vengono replicati nel sito remoto (sito B). Il contenuto dei volumi nel sito B garantisce l'ordine di scrittura per il gruppo di coerenza, permettendoti di ripristinare dal sito B in caso di disastro. Puoi accedere agli snapshot del gruppo di coerenza usando consistency group con l'ONTAP REST API e System Manager sui cluster che eseguono ONTAP 9.11.1 o versioni successive. A partire da ONTAP 9.14.1, puoi accedere agli snapshot anche tramite l'ONTAP CLI.
...
Gli snapshot dei gruppi di coerenza creati tramite l'API REST di ONTAP possono essere gestiti tramite l'interfaccia dei gruppi di coerenza di System Manager e tramite gli endpoint dell'API REST di ONTAP relativi ai gruppi di coerenza. A partire da ONTAP 9.14.1, gli snapshot dei gruppi di coerenza possono essere gestiti anche tramite la CLI di ONTAP.
