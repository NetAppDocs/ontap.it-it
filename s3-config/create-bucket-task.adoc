...
*** Se non specifichi aggregati e costituenti per un nuovo bucket, il nuovo bucket verrà aggiunto a un FlexGroup esistente, ma solo se l' `retention-mode`impostazione corrisponde all' `retention-mode`impostazione dei bucket esistenti e al tipo di FlexGroup (`compliance`, `non-snaplock`, `enterprise`). Vedi xref:../flexgroup/index.html[Gestione dei volumi FlexGroup] per maggiori informazioni.
+
