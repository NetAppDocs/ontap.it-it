...
Quando si crea un bucket S3 in un SVM abilitato per S3, un volume FlexGroup link:../s3-config/architecture.html#automatic-flexgroup-sizing-with-ontap-9-14-1-and-later["creato automaticamente"] viene creato per supportare il bucket. È possibile lasciare che ONTAP selezioni automaticamente gli aggregati e i componenti FlexGroup sottostanti (impostazione predefinita) oppure è possibile selezionare manualmente gli aggregati e i componenti FlexGroup sottostanti.
...
* https://www.netapp.com/pdf.html?item=/media/17251-tr4571a.pdf["Report tecnico NetApp 4571-a: Best practice per il volume NetApp ONTAP FlexGroup"^]
