...
+
ONTAP S3 è adatto per le funzionalità S3 sui cluster esistenti senza hardware e gestione aggiuntivi. Tuttavia, il software NetApp StorageGRID continua a essere la soluzione NetApp di punta per lo storage a oggetti. Per ulteriori informazioni, consultare link:https://docs.netapp.com/sgws-114/index.jsp["Documentazione StorageGRID"^].
