...
ONTAP S3 supporta questa intestazione aggiuntiva:
...
| DeleteBucketCors | ONTAP 9.8 


| DeleteBucketLifecycle | ONTAP 9.8 
...
A partire da ONTAP 9.19.1, ONTAP S3 supporta l'applicazione di scritture condizionali per il caricamento di oggetti e cancellazioni condizionali per la cancellazione di oggetti. Le scritture e le cancellazioni condizionali consentono di prevenire sovrascritture o cancellazioni involontarie di oggetti. Le operazioni condizionali funzionano solo se la richiesta viene effettuata utilizzando il metodo di firma AWS_V4 su una connessione TLS. Sono supportate le seguenti intestazioni condizionali e operazioni sugli oggetti:

|===
| Operazione a oggetti | Intestazione condizionale 
...
ONTAP S3 supporta questo parametro di query URI aggiuntivo:
`uploadId`
 a| 
ONTAP 9.8
...
ONTAP S3 supporta queste ulteriori intestazioni:
...
| DeleteObjects ONTAP S3 supporta questa intestazione aggiuntiva: * `x-amz-bypass-governance-retention` | ONTAP 9.11.1 
...
ONTAP S3 supporta questi parametri di query URI aggiuntivi:
...
ONTAP S3 supporta questi parametri URI aggiuntivi:
...
* ONTAP S3 aggiunge il supporto per le richieste di firma di caricamenti a blocchi utilizzando `x-amz-content-sha256:
