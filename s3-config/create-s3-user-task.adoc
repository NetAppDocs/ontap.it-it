...
Per mantenere l'accesso alle risorse S3 se la chiave di accesso originale scade, a partire da ONTAP 9.19.1, è possibile link:regenerate-access-keys.html#generate-a-second-s3-access-key["generare una seconda chiave di accesso"] per ciascun utente S3.
...
----
+
È inoltre possibile definire manualmente la propria chiave di accesso e la propria chiave segreta durante la creazione di un utente nei parametri  `-access-key` e  `-secret-key`. Ad esempio:
...
.System Manager
--
. Seleziona *Archiviazione > Macchine virtuali di archiviazione*. Seleziona la macchina virtuale di archiviazione a cui devi aggiungere un utente, seleziona *Impostazioni* e quindi seleziona image:icon_pencil.gif["Icona Edit (Modifica)"] sotto S3.
. Per aggiungere un utente, seleziona *Utenti > Aggiungi*.
...
. Seleziona *Salva*. L'utente viene creato e vengono generate una chiave di accesso e una chiave segreta per l'utente.
