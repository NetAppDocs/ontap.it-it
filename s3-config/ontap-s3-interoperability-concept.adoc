...
* FlexClone technology (volumi, file e LUN)
...
* QoS massimi (limiti)
* QoS minimi (piani)
* Criteri QoS annidati
+
link:../performance-admin/policy-group-assignment-restrictions.html["Restrizioni all'assegnazione del gruppo di policy QoS"]
...
Funzionalità aggiuntive
 a| 
* Lo spostamento dei volumi è supportato per i singoli volumi membri che compongono un FlexGroup volume.
...
* Livello cloud FabricPool (link:../s3-config/index.html["solo S3 nativo"])
* Tier locale FabricPool (link:../s3-multiprotocol/index.html["Solo volumi NAS"])
* link:../flexcache/supported-unsupported-features-concept.html#supported-features["Volumi FlexCache"] (a partire da ONTAP 9.18.1)

 a| 
* Lo spostamento dei volumi non è supportato per i volumi FlexGroup utilizzati dai bucket S3.
* FPolicy
* Qtree
* Quote
