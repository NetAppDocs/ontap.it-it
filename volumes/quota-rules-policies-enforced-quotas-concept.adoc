...
summary: Le quote sono definite in regole di quota specifiche per i volumi FlexVol. Queste regole di quota vengono raccolte in una policy di quota per una macchina virtuale di storage e attivate su ciascun volume della SVM. 
---
= Scopri le quote ONTAP, le regole relative alle quote e le policy sulle quote
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
