...
summary: Abilita la compaction inline sui sistemi FAS con aggregati Flash Pool o aggregati HDD a livello di volume o di aggregato. 
---
...
[role="lead"]
È possibile abilitare la compaction inline sui sistemi FAS con aggregati Flash Pool (ibridi) o aggregati HDD a livello di volume utilizzando il comando della shell del  `volume efficiency` cluster. La compaction dei dati è disabilitata per impostazione predefinita per i volumi creati sui sistemi FAS. Scopri di più su  `volume efficiency` nella link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=volume+efficiency["Riferimento al comando ONTAP"^].
