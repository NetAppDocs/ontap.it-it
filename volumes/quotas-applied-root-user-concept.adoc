...
summary: 'L"utente root sui client UNIX è soggetto a quote di albero, ma non a quote di utente o di gruppo. Ciò consente all"utente root di eseguire azioni per conto di altri utenti che altrimenti sarebbero impedite da una quota.' 
---
= Scopri come vengono applicate le quote ONTAP all'utente root
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
