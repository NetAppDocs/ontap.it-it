...
---
= Scopri come creare un volume ONTAP FlexClone da fonte di SnapMirror o volume di destinazione
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
È possibile creare un volume FlexClone dal volume di origine o dal volume di destinazione in una relazione di volume SnapMirror esistente. Tuttavia, se si utilizza una versione di ONTAP precedente alla ONTAP 9.12.1, con relazioni di tipo DP che utilizzano la policy  `DPDefault`, questa operazione potrebbe impedire il completamento corretto delle future operazioni di replica SnapMirror.
...
Se si utilizzano relazioni XDP con ONTAP 9.12.1 o versioni successive con la policy  `MirrorAllSnapshots`, non si dovrebbe riscontrare questo problema.

Informazioni correlate: link:../data-protection/convert-snapmirror-version-flexible-task.html["Convertire una relazione di tipo ONTAP SnapMirror DP esistente in XDP"]
