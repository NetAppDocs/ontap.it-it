...
.Fasi
. In System Manager, selezionare *Storage > Volumes*.
+
Viene visualizzato l'elenco dei volumi.

+
Gli amministratori di SVM visualizzano un set limitato di opzioni e potrebbero non essere autorizzati ad aggiungere volumi.
...
Aggiungere un volume
 a| 
Seleziona image:icon_add_blue_bg.gif["Icona Add (Aggiungi)"]. Vedi link:../task_admin_add_a_volume.html["Aggiungere un volume"].
...
* Seleziona image:icon_delete_with_can_white_bg.gif["Icona Elimina"] per eliminare i volumi selezionati.
* Selezionare image:icon_protect.gif["Icona di protezione"] per assegnare una policy di protezione ai volumi selezionati.
* Selezionare image:icon-more-kebab-white-bg.gif["icona altro"] per selezionare una delle seguenti azioni da eseguire su tutti i volumi selezionati:
+
** Attiva quota
** Non in linea
** Sposta
...
Gestire un singolo volume
 a| 
Accanto al volume, seleziona image:icon_kabob.gif["Icona delle opzioni di menu"], quindi seleziona una delle seguenti azioni da eseguire:
...
Seleziona image:icon-edit-pencil-blue-outline.png["Icona Edit (Modifica)"] accanto al nome del volume e poi modifica il nome del volume.
