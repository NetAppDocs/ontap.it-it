...
.. Immettere un nome, selezionare la VM di storage e immettere una dimensione. Vengono elencate solo le VM di storage configurate con i protocolli NFS e SMB. Se è disponibile una sola VM di storage configurata con i protocolli NFS e SMB, il campo *Storage VM* non viene visualizzato.
.. Fare clic su *altre opzioni* e selezionare *Condividi via NFS*.
+
