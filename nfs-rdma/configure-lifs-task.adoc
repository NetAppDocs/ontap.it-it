...
NOTE: Ogni porta RDMA può avere un massimo di 127 LIF. Quando aggiungi altre LIF, o quando una LIF effettua il failover su una porta che supera tale limite, le LIF appena aggiunte o la LIF di failover non possono gestire il traffico RDMA.
...
. Seleziona la macchina virtuale di archiviazione e il nodo principale. Assegna un *nome*, un *indirizzo IP* e una *maschera di sottorete*.
...
** Il `-rdma-protocols` parametro accetta un elenco, che per impostazione predefinita è vuoto. Quando `roce` viene aggiunto come valore, il LIF può essere configurato solo sulle porte che supportano l'offload RoCE, influenzando sia la migrazione che il failover del LIF.
