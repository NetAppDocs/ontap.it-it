...
summary: 'Il server SMB supporta gli oggetti Criteri di gruppo (GPO), un insieme di regole note come attributi dei criteri di gruppo che si applicano ai computer in un ambiente Active Directory. È possibile utilizzare i GPO per gestire centralmente le impostazioni di tutte le macchine virtuali di storage del cluster appartenenti allo stesso dominio Active Directory.' 
---
