...
summary: A partire da ONTAP 9, è possibile configurare la firma e la sigillatura per abilitare la sicurezza delle sessioni LDAP sulle query a un server Active Directory. È necessario configurare le impostazioni di sicurezza del server CIFS sulla macchina virtuale di storage in modo che corrispondano a quelle del LDAP server. 
---
