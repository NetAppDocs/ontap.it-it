...
summary: 'Prima che gli utenti possano creare connessioni SMB per accedere ai dati contenuti nell"SVM, devono essere autenticati dal dominio a cui appartiene il server SMB. Il server SMB supporta due metodi di autenticazione, Kerberos e NTLM. Kerberos è il metodo predefinito utilizzato per autenticare gli utenti del dominio.' 
---
