...
+
Il pass-through di ODX copy offload supporta le operazioni di copia avviate da un sistema operativo guest Hyper-V. Il sistema operativo guest deve supportare ODX e utilizzare dischi virtuali SCSI supportati da storage SMB o SAN compatibile con ODX. I dischi virtuali IDE non supportano il pass-through ODX. Lo storage di origine e la destinazione devono trovarsi sullo stesso cluster.

+
Per ulteriori informazioni, vedi link:use-cases-odx-concept.html["Casi d'uso per ODX sui server ONTAP SMB"].
