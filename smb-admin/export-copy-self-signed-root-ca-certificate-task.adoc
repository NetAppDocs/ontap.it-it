...
summary: Per utilizzare LDAP su SSL/TLS per proteggere la comunicazione con Active Directory, è necessario innanzitutto esportare una copia del certificato CA radice autofirmato del Servizio certificati di Active Directory in un file di certificato e convertirlo in un file di testo ASCII. Questo file di testo viene utilizzato da ONTAP per installare il certificato sulla storage virtual machine. 
---
