...
summary: 'A partire da ONTAP 9.3, è possibile modificare il processo predefinito mediante il quale vengono individuati i controller di dominio. Ciò consente di limitare la discovery al proprio sito o a un pool di DC preferiti, il che può portare a miglioramenti delle prestazioni a seconda dell"ambiente.' 
---
...
+
Il `default-site` parametro per il server SMB può essere definito per utilizzare questa modalità con LIF che non sono assegnate a un sito in siti e servizi. A partire da ONTAP 9.13.1, il sito predefinito può essere utilizzato anche come fallback se ONTAP non riesce a individuare le informazioni sul sito per un dominio attendibile.
