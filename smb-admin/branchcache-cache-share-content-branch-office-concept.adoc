...
summary: 'BranchCache è stato sviluppato da Microsoft per consentire la memorizzazione nella cache dei contenuti sui computer locali dei client richiedenti. L"implementazione ONTAP di BranchCache può ridurre l"utilizzo della rete geografica e fornire tempi di risposta di accesso migliorati quando gli utenti di una filiale accedono ai contenuti archiviati su macchine virtuali di storage tramite SMB.' 
---
