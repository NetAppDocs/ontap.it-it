...
summary: Ci sono alcune linee guida che devi tenere a mente prima di applicare le policy file-directory sulla destinazione di disaster recovery della storage virtual machine in una configurazione di eliminazione ID, se la configurazione della policy file-directory utilizza utenti o gruppi locali nel descrittore di sicurezza o nelle voci DACL o SACL. 
---
