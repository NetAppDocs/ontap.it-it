...
summary: 'Quando un"applicazione per il backup in esecuzione su Windows incontra un collegamento simbolico in stile Unix, il collegamento viene seguito e i dati vengono sottoposti a backup. A partire da ONTAP 9.15.1, è possibile scegliere di eseguire il backup dei collegamenti simbolici anziché dei dati. Questa funzionalità è completamente supportata con volumi ONTAP FlexGroup e FlexVols.' 
---
...
Oltre alla CLI di ONTAP, puoi anche gestire questa funzionalità usando la ONTAP REST API. Vedi  https://docs.netapp.com/us-en/ontap-automation/whats-new.html["Novità dell'API REST e dell'automazione di ONTAP"^] per maggiori informazioni. La configurazione che determina come ONTAP gestisce i symlink in stile Unix deve essere eseguita separatamente per ogni SVM.
