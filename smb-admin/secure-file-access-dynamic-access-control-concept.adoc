...
summary: 'È possibile proteggere l"accesso utilizzando il Controllo dinamico degli accessi e creando policy di accesso centralizzate in Active Directory e applicandole a file e cartelle sulle SVM tramite oggetti Criteri di gruppo applicati. È possibile configurare l"auditing per utilizzare eventi di staging delle policy di accesso centralizzate per vedere gli effetti delle modifiche alle policy di accesso centralizzate prima di applicarle.' 
---
