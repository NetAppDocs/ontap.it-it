...
summary: Se si desidera che il DNS server integrato con Active Directory registri dinamicamente i record DNS del server SMB in DNS quando si sposta il server SMB in un altro dominio, è necessario modificare il DNS dinamico sulla storage virtual machine prima di spostare il server SMB. 
---
