...
summary: Per sfruttare al massimo la sicurezza offerta dalle comunicazioni basate su Kerberos, è possibile abilitare la crittografia AES-256 e AES-128 sul server SMB. Se non si desidera che il server SMB selezioni i tipi di crittografia AES per le comunicazioni basate su Kerberos con il KDC di Active Directory, è possibile disabilitare la crittografia AES. Per impostazione predefinita, la crittografia AES è disabilitata. 
---
...
Quando modifichi il  `-advertised-enc-types` parametro, i tipi di crittografia specificati sostituiscono tutti i tipi configurati in precedenza. Tutte le impostazioni precedenti dei tipi di crittografia vengono rimosse.

Per migliorare la sicurezza, la storage virtual machine (SVM) cambia la password del proprio account macchina in AD ogni volta che viene modificata l'opzione di sicurezza AES. La modifica della password potrebbe richiedere le credenziali amministrative di AD per l'unità organizzativa (OU) che contiene l'account macchina. Viene visualizzata una richiesta di credenziali solo quando ONTAP deve reimpostare la password dell'account macchina in Active Directory e la configurazione di AD richiede le credenziali amministrative per l'unità organizzativa che contiene l'account macchina. Se il tuo AD è sincronizzato e la reimpostazione automatica è abilitata, non viene visualizzata alcuna richiesta.
