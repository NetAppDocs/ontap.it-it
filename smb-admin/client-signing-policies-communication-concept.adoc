...
[NOTE]
====
Microsoft non consiglia più di utilizzare  `Digitally sign communications (if client agrees)` o  `Digitally sign communications (if server agrees)` le impostazioni di Criteri di gruppo. Microsoft non consiglia più nemmeno di utilizzare le  `EnableSecuritySignature` impostazioni del registro di sistema. Queste opzioni influiscono solo sul comportamento di SMB 1 e possono essere sostituite dall' `Digitally sign communications (always)` impostazione di Criteri di gruppo o dall' `RequireSecuritySignature` impostazione del registro di sistema. Puoi anche ottenere maggiori informazioni da link:http://blogs.technet.com/b/josebda/archive/2010/12/01/the-basics-of-smb-signing-covering-both-smb1-and-smb2.aspx["Blog Microsoft: Le basi della firma SMB (che copre sia SMB1 che SMB2)"^].
