...
summary: Esistono diversi motivi per creare utenti e gruppi locali sulla macchina virtuale di storage. Ad esempio, è possibile accedere a un server SMB utilizzando un account utente locale se i domain controller non sono disponibili, si potrebbe voler utilizzare i gruppi locali per assegnare privilegi oppure il server SMB si trova in un workgroup. 
---
