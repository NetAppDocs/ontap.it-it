...
summary: Le prestazioni dei carichi di lavoro iniziano a risentire della latenza quando la quantità di lavoro su un nodo supera le risorse disponibili. È possibile gestire un nodo sovraccarico aumentando le risorse disponibili, ad esempio aggiornando i dischi o la CPU, oppure riducendo il carico, ad esempio spostando volumi o LUN su nodi diversi a seconda delle necessità. 
---
= Scopri il bilanciamento del carico in ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
