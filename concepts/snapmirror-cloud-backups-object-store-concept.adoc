...
[role="lead"]
_SnapMirror cloud_ è una tecnologia di backup e ripristino progettata per gli utenti ONTAP che desiderano trasferire i propri flussi di lavoro di protezione dei dati nel cloud o in una soluzione di storage a oggetti on-premise. Le organizzazioni che abbandonano le architetture legacy di backup su nastro possono utilizzare lo storage a oggetti come repository alternativo per la conservazione e l'archiviazione dei dati a lungo termine. SnapMirror cloud fornisce la replica da ONTAP a storage a oggetti come parte di una strategia di backup incrementale continuo.
