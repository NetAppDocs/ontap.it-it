...
---
= Scopri l'autenticazione e l'autorizzazione del client in ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP supporta il Security Assertion Markup Language (SAML) per l'autenticazione degli utenti remoti. Sono supportati diversi provider di identità diffusi (IdPs). Per ulteriori informazioni sui IdPs supportati e per le istruzioni su come abilitare l'autenticazione SAML, fare riferimento a link:../system-admin/configure-saml-authentication-task.html["Configurare l'autenticazione SAML"].
...
Il supporto per il framework Open Authorization (OAuth 2.0) è disponibile a partire da ONTAP 9.14. Puoi usare OAuth 2.0 per prendere decisioni di autorizzazione e controllo degli accessi solo quando il client utilizza l'ONTAP REST API per accedere a ONTAP. Tuttavia, puoi configurare e abilitare la funzionalità tramite qualsiasi interfaccia amministrativa di ONTAP, inclusi ONTAP CLI, System Manager e ONTAP REST API.
