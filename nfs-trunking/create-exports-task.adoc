...
Requisiti di esportazione ONTAP:

* La policy di esportazione predefinita del volume root della SVM deve includere una regola che consenta a tutti i client l'accesso aperto tramite NFS.
+
Scopri di più sulink:../nfs-config/open-export-policy-svm-root-volume-task.html["Accesso client NFS sull'SVM ONTAP."]

* È necessario aggiungere la stessa regola alla policy predefinita se hai creato una policy di esportazione personalizzata.
+
Scopri di più sulink:../nfs-admin/default-export-policy-svms-concept.html["Policy di esportazione predefinite per SVM NFS ONTAP."]
