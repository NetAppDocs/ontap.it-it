...
| Miglioramenti a SnapMirror Business Continuity (SM-BC)  a| 
* link:../smbc/smbc_admin_add_and_remove_volumes_in_consistency_groups.html["Aggiunta senza interruzioni di volumi a un gruppo di coerenza con una relazione SM-BC attiva."]
* link:../smbc/supported-configurations-reference.html#ndmp-restore["Utilizzare il ripristino NDMP con SM-BC"].
...
* xref:../use-cases-restrictions-concept.html#multi-admin-verification-with-volumes-protected-with-arp[Verifica della funzionalità degli amministratori multipli con la protezione autonoma dal ransomware]
