...
| Aggregato RAID | A partire da ONTAP 9.12.1, il controller di sistema non si arresta per impostazione predefinita dopo 24 ore in caso di degrado di un aggregato. Se un utente modifica il `raid.timeout` in alternativa, il controller di sistema continuerà a spegnersi dopo la scadenza di `raid.timeout` ore. | 9.12.1 
