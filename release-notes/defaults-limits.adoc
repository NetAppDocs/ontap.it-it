...
| Protezione autonoma contro i ransomware (ARP/AI) | ARP/AI viene abilitato automaticamente per impostazione predefinita su tutti i nuovi volumi dopo un grace period di 12 ore sulle piattaforme supportate. | ONTAP 9.18.1 
...
| Tempo di vita (TTL) delle credenziali positive memorizzate nella cache per le SVM ONTAP | A partire da ONTAP 9.17.1, il time-to-live (TTL) delle credenziali positive memorizzate nella cache per gli SVM di ONTAP è passato da 24 ore (86,400,000 millisecondi) a 1 ora (3,600,000 millisecondi). | ONTAP 9.17.1 
...
| Mirror per la condivisione del carico | Quando si crea una relazione mirror di condivisione del carico, l'SVM non può avere un limite di archiviazione abilitato. | ONTAP 9.16.1 
...
| Cloud SnapMirror | SnapMirror cloud aumenta il numero di bucket S3 supportati da 6 a 100. | ONTAP 9.19.1 


| limite di controllo del flusso della connessione S3 | Il limite di controllo del flusso per le connessioni ONTAP S3 è stato ridotto da 2 MB a 256 KB per le istanze cloud con una memoria pari o inferiore a 32 GB. | ONTAP 9.19.1 


| Timeout di connessione inattiva S3 | Il tempo di timeout della connessione S3 inattiva è stato ridotto da 20 minuti a 5 minuti. | ONTAP 9.19.1 
...
| Peering del cluster | Il limite di cluster peer supportati è stato aumentato da 255 a 400, migliorando la scalabilità per gli ambienti con un elevato numero di relazioni di cluster peer. | ONTAP 9.14.1 
...
* Il numero massimo di costituenti in un FlexGroup non può superare 24.
* I limiti di volume per SVM-DR sono 500 per nodo e 1000 per coppia HA (inclusi i volumi FlexVol e i componenti FlexGroup).
+

NOTE: I limiti sono specifici della piattaforma. Dovresti verificare i limiti corretti per la tua configurazione andando su link:https://hwu.netapp.com["NetApp Hardware Universe"^] e selezionando la tua piattaforma e la versione di ONTAP.
...
| Trasferimento simultaneo di SnapMirror | Il numero massimo di trasferimenti simultanei asincroni a livello di volume è aumentato da 100 a 200.  I trasferimenti SnapMirror Cloud sono aumentati da 32 a 100 sui sistemi di fascia alta e da 6 a 20 trasferimenti SnapMirror sui sistemi di fascia bassa. | ONTAP 9.8 
