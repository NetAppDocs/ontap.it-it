...
xref:../smb-admin/windows-backup-symlinks.html[Applicazioni di backup di Windows e collegamenti simbolici in stile Unix sui server]
 a| 
Quando un'applicazione per il backup di Windows incontra un collegamento simbolico (symlink) in stile Unix, il collegamento viene seguito e i dati effettivi vengono restituiti da ONTAP e sottoposti a backup. A partire da ONTAP 9.15.1, hai anche la possibilità di eseguire il backup del symlink stesso invece dei dati a cui punta. Questo può offrire diversi vantaggi, tra cui un miglioramento delle prestazioni delle tue applicazioni per il backup. Puoi abilitare la funzionalità usando la ONTAP CLI o la ONTAP REST API.
...
I gruppi di coerenza che utilizzano la protezione asincrona di SnapMirror ora supportano fino a 80 volumi nel gruppo di coerenza.
 ...
 a| 
xref:../consistency-groups/configure-task.html[Supporto per il livello di privilegio amministrativo per le operazioni ONTAP REST API e ONTAP CLI con consistency group]
 a| 
Le operazioni ONTAP CLI e ONTAP REST API per i gruppi di coerenza sono ora supportate al livello di privilegio amministrativo.
...
xref:../concepts/ontap-and-vmware.html[Prenotazioni persistenti per volumi virtuali VMware con Windows Server failover Clustering]
 a| 
ONTAP attualmente supporta i volumi virtuali VMware (vVols) e anche le prenotazioni persistenti con LUN tradizionali. A partire da ONTAP 9.15.1, puoi anche creare una prenotazione persistente con un vVol. Il supporto per questa funzionalità è implementato in ONTAP tools for VMware vSphere 9. È supportato solo in un Windows Server Failover Cluster (WSFC), cioè un gruppo di macchine virtuali Windows in cluster.
