...
ONTAP 9.15.1 offre funzioni nuove e migliorate nel campo della gestione della sicurezza, della protezione dei dati e del supporto dei workload NAS. Per un elenco completo delle nuove funzioni e dei miglioramenti, vedere xref:9151-reference.adoc[Novità di ONTAP 9.15.1].
...
+
Proteggi i dati "via cavo" a livello di protocollo con configurazione semplificata rispetto ad altre tecnologie come IPSec e NFS Kerberos. Questa funzione è attualmente inclusa come anteprima pubblica. Per ulteriori informazioni su questa funzionalità, contatta il tuo team di vendita.
...
* xref:../flexcache/flexcache-writeback-enable-task.html[Writeback di FlexCache]
+
