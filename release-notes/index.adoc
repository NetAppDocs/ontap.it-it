...
* Scopri https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["nuove e migliorate funzionalità del software ONTAP per i sistemi di storage NetApp AFX"^].
* Scopri https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["nuove e migliorate funzionalità del software ONTAP per i sistemi di storage NetApp ASA r2"^].
...
== Novità principali di ONTAP 9.19.1

ONTAP 9.19.1 offre nuove funzionalità e miglioramenti in materia di sicurezza, scalabilità e gestione dello storage per supportare la modernizzazione dell'infrastruttura dati. Questi miglioramenti ti aiutano a scalare, operare in modo più efficiente e proteggere i dati per il futuro. Per un elenco completo delle nuove funzionalità e dei miglioramenti, vedi xref:whats-new-9191.adoc[Novità di ONTAP 9.19.1].

* xref:../performance-admin/guarantee-throughput-qos-task.html[Policy di burst QoS configurabili]
+
Le policy di burst QoS configurabili consentono picchi di prestazioni controllati e a tempo limitato al di sopra dei limiti QoS, migliorando la reattività del carico di lavoro durante i picchi senza sacrificare l'equità o richiedere un over-provisioning.

* xref:../nfs-admin/enable-nfs-over-tls-task.html[NFS su TLS 1.3]
+
Protegge i dati NFS in transito con la crittografia TLS 1.3 basata su standard, aiutando a rafforzare la protezione e a semplificare l'adozione di NFS crittografato su scala aziendale. 

* xref:../s3-config/regenerate-access-keys.html[Più chiavi di accesso S3 per utente]
+
Aggiunge il supporto per più chiavi di accesso S3 attive per utente, consentendo una rotazione continua delle credenziali senza downtime applicativo e aiutando i clienti a rafforzare la sicurezza e la conformità mantenendo un accesso ininterrotto ai carichi di lavoro degli oggetti.

* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror active sync per NAS]
+
Grazie al supporto NAS, SnapMirror active sync è ora una soluzione unificata per la continuità operativa che offre uno RPO pari a zero con failover trasparente. 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC collegato direttamente]
+
Introduce il supporto sicuro per le schede FC HBA, consentendo una connettività host affidabile e ponendo le basi per la sicurezza FC in volo supportata dall'hardware, mantenendo le prestazioni e soddisfacendo i requisiti di conformità aziendali. 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[Failover LIF attivo-attivo unificato]
+
I sistemi AFF offrono multipathing attivo-attivo con failover LIF automatico, mantenendo tutti i percorsi SAN attivi-ottimizzati così che l'I/O dell'host continui immediatamente durante il failover. 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["Supporto iSCSI in FLI"^]
+
Aggiunge il supporto iSCSI per l'importazione di LUN esteri (FLI), consentendoti di migrare dati a blocchi da sistemi storage di terze parti a ONTAP utilizzando la connettività iSCSI esistente con interruzioni minime. 

* xref:../s3-config/create-modify-bucket-policy-task.html[Scritture e cancellazioni condizionali su S3]
+
Consente il supporto per scritture ed eliminazioni condizionali con l'API S3, permettendo alle applicazioni di applicare controlli di coerenza dei dati durante gli aggiornamenti e le eliminazioni degli oggetti, riducendo il rischio di sovrascritture o eliminazioni accidentali in carichi di lavoro di oggetti concorrenti e distribuiti.

* xref:../concepts/path-failover-concept.html#nas-path-failover[Failover LIF combinato]
+
...
* xref:../networking/improve-tcp-wan-performance.html[Prestazioni WAN migliorate]
+
Prestazioni WAN ottimizzate con TCP RACK migliorano throughput e resilienza su reti congestionate, accelerando l'accesso remoto ai file e i servizi dati tra siti. 

* xref:../networking/configure-tls-offload.html[kTLS con accelerazione hardware per una crittografia del cluster backend più rapida]
+
Migliora la crittografia del cluster backend introducendo kTLS con accelerazione hardware, scaricando l'elaborazione della crittografia sulle SmartNIC supportate per offrire prestazioni superiori e un minore carico sulla CPU, mantenendo sempre una comunicazione sicura e sempre attiva tra cluster e traffico HA. 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[Rilevamento ransomware basato sull'intelligenza artificiale per carichi di lavoro replicati in modo sincrono]
+
Estende la protezione autonoma contro i ransomware (ARP/AI) a SnapMirror synchronous per ambienti SAN e NAS sincroni e SnapMirror active sync SAN, preservando i segnali di rilevamento e ripristino nelle architetture a RPO zero. 
...
+
Fornisce una GUI intuitiva per il tenant per la gestione di SVM, eliminando la necessità di un'interfaccia CLI complessa. 
...
== Punti salienti di ONTAP 9.18.1

ONTAP 9.18.1 offre funzionalità nuove e migliorate nei settori della sicurezza, della scalabilità e della gestione dell'archiviazione per supportare la modernizzazione dell'infrastruttura dati.  Questi miglioramenti ti aiutano a scalare, a operare in modo più efficiente e a proteggere i dati per il futuro.  Per un elenco completo delle nuove funzionalità e dei miglioramenti, vederexref:whats-new-9181.adoc[Novità di ONTAP 9.18.1] .

* xref:../networking/configure-backend-encryption.html[Crittografia HA del cluster backend]
+
Possibilità di utilizzare mTLS per crittografare la rete del cluster backend.

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[Supporto PQC per i dati in transito]
+
Capacità di utilizzare algoritmi post-quantistici per i dati in transito.

* xref:../networking/ipsec-prepare.html[Supporto IPv6 IPSec con offload HW]
+
Supporto per IPSec con offload hardware con indirizzo IPv6.

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[Supporto ARP/AI per volumi FlexGroup]
+
...
* xref:../san-admin/nvme-copy-offload.html[Copia Offload per NVMe]
+
Accelera i trasferimenti di dati di grandi dimensioni scaricando le operazioni di copia direttamente sullo storage, riducendo il carico della CPU host e della rete.

* xref:../s3-snapshots/list-restore-s3-snapshot.html[Ripristino degli snapshot del bucket S3 basato sullo storage]
+
...
ONTAP 9.17.1 introduce innovazioni nella protezione autonoma contro i ransomware basata sull'intelligenza artificiale per SAN e aggiunge il supporto host front-end NVMe con SnapMirror ActiveSync e gestione gerarchica dei gruppi di coerenza.  Questi miglioramenti rafforzano la leadership di NetApp nella sicurezza dei dati e nella resilienza informatica, migliorando le prestazioni, la scalabilità, l'efficienza e la continuità aziendale ONTAP . Per un elenco completo delle nuove funzionalità e dei miglioramenti, consultare xref:whats-new-9171.adoc[Novità di ONTAP 9.17.1] .

* xref:../anti-ransomware/index.html[Supporto per SAN con protezione autonoma da ransomware]
+
ARP/AI supporta i volumi SAN con rilevamento delle anomalie basato sulla crittografia, introduce nuovi comandi per statistiche di entropia dettagliate e unifica la messaggistica di protezione dai ransomware in System Manager, che in precedenza era focalizzata sui NAS. Soglie di rilevamento configurabili e una conservazione degli snapshot più deterministica offrono maggiore flessibilità per diversi carichi di lavoro.

* xref:../system-admin/use-hsts-task.html[Supporto HTTP Strict Transport Security (HSTS)]
+
ONTAP 9.17.1 supporta HTTP Strict Transport Security per i servizi Web, consentendo l'applicazione di comunicazioni HTTPS sicure tra il browser di un utente e ONTAP.

* xref:../networking/ipsec-prepare.html[Offload hardware IPsec con gruppi di aggregazione di link]
+
ONTAP 9.17.1 supporta l'offload hardware IPsec per i gruppi di aggregazione di link, estendendo il supporto all'offload hardware introdotto in ONTAP 9.16.1.
...
+
SnapMirror ActiveSync aggiunge il supporto per l'accesso NVMe per carichi di lavoro VMware con accesso host NVMe/TCP e NVMe/FC per cluster ONTAP a due nodi.
...
+
ONTAP 9.16.1 aggiunge ulteriori comandi al framework MAV per una maggiore protezione dagli utenti interni malintenzionati. Questi miglioramenti includono numerosi comandi per la gestione dei Consistency Group (CG), VScan e Autonomous Ransomware Protection (ARP/AI), e comandi di configurazione NVMe.

* xref:../anti-ransomware/index.html[Protezione autonoma dal ransomware con miglioramenti ai (ARP/ai)]
+
La protezione autonoma contro i ransomware è stata aggiornata con nuove funzionalità di intelligenza artificiale, che le consentono di rilevare e rispondere agli attacchi ransomware con una precisione e un richiamo del 99%. Poiché l'IA è addestrata su un set di dati completo, non è più necessario un periodo di apprendimento per ARP/AI in esecuzione sui volumi FlexVol e ARP/AI si avvia immediatamente in modalità attiva. ARP/AI introduce anche una funzionalità di aggiornamento automatico indipendente da un aggiornamento ONTAP per garantire una protezione e una resilienza costanti contro le minacce più recenti.
...
+
La deallocazione dello spazio (detta anche "hole punching" e "unmap") è ora supportata per gli spazi dei nomi NVMe.  La deallocazione dello spazio consente ai volumi con thin provisioning e agli spazi dei nomi NVMe di recuperare spazio inutilizzato quando i dati vengono eliminati dall'applicazione host.  Ciò migliora notevolmente l'efficienza complessiva dell'archiviazione, soprattutto con i file system che presentano un elevato turnover dei dati.
...
+
A partire da ONTAP 9.15.1, hai anche l'opzione di eseguire il backup del collegamento simbolico stesso invece che dei dati a cui punta. Questo può offrire diversi vantaggi, incluso un miglioramento delle prestazioni delle tue applicazioni per il backup. Puoi abilitare la funzionalità usando la ONTAP CLI o la ONTAP REST API.
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[Supporto per la verifica multi-admin]
+
...
+
Se i componenti di FlexGroup diventano sbilanciati, FlexGroup può essere ribilanciato e gestito senza interruzioni tramite ONTAP CLI, ONTAP REST API e System Manager. Per prestazioni ottimali, i membri costituenti di un FlexGroup dovrebbero avere la capacità utilizzata distribuita uniformemente.
...
* Miglioramenti di System Manager
+
System Manager scarica automaticamente gli aggiornamenti del firmware per dischi, shelf e processori di servizio, oltre a fornire nuove integrazioni con Active IQ Digital Advisor (noto anche come Digital Advisor), NetApp Console e la gestione dei certificati.  Questi miglioramenti semplificano l'amministrazione e mantengono la continuità aziendale.
