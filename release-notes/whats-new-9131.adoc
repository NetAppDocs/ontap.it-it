...
| xref:../snaplock/snapshot-lock-concept.html#enable-snapshot-copy-locking-when-creating-a-volume[Blocco delle istantanee antimanomissione] | Puoi utilizzare System Manager per bloccare uno snapshot su un volume non SnapLock e fornire protezione contro gli attacchi ransomware. 
