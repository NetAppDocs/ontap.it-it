...
xref:../anti-ransomware/index.html[Protezione autonoma dal ransomware con miglioramenti ai (ARP/ai)]
 a| 
La protezione autonoma contro i ransomware è stata aggiornata con nuove funzionalità di intelligenza artificiale, che le consentono di rilevare e rispondere agli attacchi ransomware con una precisione e un richiamo del 99%. Poiché l'AI è addestrata su un set di dati completo, non è più necessario un periodo di apprendimento per ARP/AI in esecuzione sui volumi FlexVol e ARP/AI parte subito in modalità attiva. ARP/AI include anche una funzionalità di aggiornamento automatico per garantire una protezione costante e resilienza contro le minacce più recenti.
...
| Aggiornare | Descrizione 
 ...
 a| 
xref:../authentication-access-control/webauthn-mfa-overview.html[Supporto per l'autenticazione a più fattori resistente al phishing WebAuthn in System Manager]
 a| 
