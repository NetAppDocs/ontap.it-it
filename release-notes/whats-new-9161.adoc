...
| Aggiornare | Descrizione 
 ...
 a| 
xref:../networking/configure_virtual_ip_@vip@_lifs.html#set-up-border-gateway-protocol-bgp[Supporto di autenticazione MD5 per i gruppi di peer BGP]
 a| 
...
xref:../s3-multiprotocol/index.html[Il bucket S3 multiprotocollo supporta il caricamento multiparte]
 a| 
Con il caricamento multiparte, puoi caricare un singolo oggetto come set di parti nel bucket multiprotocollo S3.
...
NOTE: La funzione ARP/ai attualmente supporta solo NAS. Sebbene la funzionalità di aggiornamento automatico mostri la disponibilità di nuovi file di sicurezza per la distribuzione in System Manager, questi aggiornamenti sono applicabili solo per la protezione del carico di lavoro NAS.
 ...
 a| 
xref:../nvme/set-up-tls-secure-channel-nvme-task.html[Supporto per la crittografia in-flight per il traffico dati inviato da e verso i dispositivi di storage NVMe]
 a| 
ONTAP ora supporta la crittografia in-flight per il traffico dati inviato in rete da e verso i dispositivi di storage NVMe.
...
xref:../flexgroup/enable-adv-capacity-flexgroup-task.html[Supporto per la distribuzione avanzata della capacità FlexGroup]
 a| 
Se abilitato, il bilanciamento avanzato della capacità distribuisce i dati tra i volumi membri FlexGroup quando file di grandi dimensioni crescono e occupano spazio su un volume membro.
...
xref:../svm-migrate/index.html[Supporto di mobilità dei dati SVM per la migrazione delle configurazioni MetroCluster]
 a| 
Ora ONTAP supporta la migrazione di una SVM da una coppia ha non MetroCluster a una configurazione MetroCluster o da una configurazione MetroCluster a una coppia ha non MetroCluster. Non è possibile migrare una SVM da una configurazione MetroCluster a un'altra configurazione MetroCluster
