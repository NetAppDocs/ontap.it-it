...
Puoi definire un server di autorizzazione per ONTAP usando una qualsiasi delle interfacce amministrative, inclusi ONTAP CLI, System Manager o ONTAP REST API. Ad esempio, con ONTAP CLI usi il comando  `security oauth2 client create`.
...
I token di accesso OAuth 2.0 emessi dai server di autorizzazione vengono verificati da ONTAP e utilizzati per prendere decisioni di accesso in base al ruolo per le richieste dei client ONTAP REST API.



=== Acquisizione di un token di accesso

Devi ottenere un token di accesso da un server di autorizzazione definito per il cluster ONTAP dove usi l'ONTAP REST API. Per ottenere un token, devi contattare direttamente il server di autorizzazione.
