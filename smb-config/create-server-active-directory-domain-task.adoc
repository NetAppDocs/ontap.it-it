...
Per creare il server SMB, sono necessari i seguenti privilegi minimi sull'unità organizzativa (OU):

* Creare oggetti computer
* Elimina oggetti computer
* Reimposta password
* Restrizioni di lettura e scrittura dell'account
* Scrittura convalidata su host name DNS
* Scrittura convalidata nei service principal names
* Leggi msDS-SupportedEncryptedTypes
* Scrivi msDS-SupportedEncryptedTypes
...
* È possibile scegliere di aggiungere il server SMB a un'unità organizzativa diversa utilizzando l'opzione  `-ou`.
* A partire da ONTAP 9.13.1, puoi configurare un sito predefinito per il server SMB. ONTAP può utilizzare questo sito quando un LIF non appartiene a un sito e come fallback se il rilevamento del sito non riesce per un trusted domain durante l'individuazione dei domain controller del trusted domain.
...
. Crea il server SMB in un dominio AD: `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
...
A partire da ONTAP 9.13.1, il sito predefinito del server SMB viene utilizzato anche come fallback se ONTAP non riesce a rilevare le informazioni sul sito per un dominio attendibile.
