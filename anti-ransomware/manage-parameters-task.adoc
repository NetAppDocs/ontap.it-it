...
Determinati volumi e carichi di lavoro richiedono parametri di rilevamento diversi.  Ad esempio, il volume abilitato per ARP potrebbe ospitare numerosi tipi di estensioni di file, nel qual caso potrebbe essere necessario modificare il conteggio soglia per le estensioni di file mai viste prima su un numero maggiore del valore predefinito di 20 oppure disabilitare gli avvisi basati sulle estensioni di file mai viste prima.  A partire da ONTAP 9.11.1, è possibile modificare i parametri di rilevamento degli attacchi in modo che si adattino meglio ai carichi di lavoro specifici.
...
NOTE: Per ridurre l'elevato numero di falsi positivi, accedere a *Storage > Volumes > Security > Configure workload characteristics* e disabilitare *Monitor new file types*. Questa impostazione è disabilitata per impostazione predefinita in ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 e versioni successive. A partire da ONTAP 9.18.1 P2, System Manager non visualizza più le caratteristiche del carico di lavoro ARP, incluse le statistiche di baseline e di surge. Le statistiche di surge rimangono disponibili tramite le opzioni ONTAP CLI e REST API per analisi avanzate.
...
A seconda dei comportamenti previsti del volume abilitato per ARP, potrebbe essere necessario modificare i parametri di rilevamento degli attacchi.
