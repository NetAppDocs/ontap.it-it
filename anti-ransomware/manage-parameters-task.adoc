...
NOTE: Per ridurre l'elevato numero di falsi positivi, vai su *Archiviazione > Volumi > Sicurezza > Configura caratteristiche del carico di lavoro* e disabilita *Monitora nuovi tipi di file*. Questa impostazione è disabilitata per impostazione predefinita in ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 e versioni successive. A partire da ONTAP 9.18.1 P2, System Manager non mostra più le caratteristiche del carico di lavoro ARP, incluse le statistiche di baseline e di picco. Le statistiche di picco restano disponibili tramite ONTAP CLI e ONTAP REST API per analisi avanzate.
