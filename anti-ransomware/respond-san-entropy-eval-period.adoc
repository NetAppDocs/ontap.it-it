...
[role="lead"]
A partire da ONTAP 9.17.1, ARP richiede un periodo di valutazione per determinare se i livelli di entropia per i carichi di lavoro dei volumi SAN siano idonei alla protezione da ransomware. Dopo aver abilitato ARP su un volume SAN, ARP/AI monitora e protegge attivamente il volume durante il periodo di valutazione, determinando contemporaneamente una soglia di crittografia ottimale.  Durante il periodo di valutazione è possibile effettuare rilevamenti e avvisi utilizzando una soglia conservativa, mentre vengono stabilite le soglie di base. ARP distingue tra carichi di lavoro idonei e non idonei nel volume SAN valutato e, se i carichi di lavoro vengono ritenuti idonei per la protezione, imposta automaticamente una soglia di crittografia in base alle statistiche del periodo di valutazione.
