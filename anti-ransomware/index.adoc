...
[role="lead"]
A partire da ONTAP 9.10.1, gli amministratori di ONTAP possono abilitare Autonomous Ransomware Protection (ARP) per eseguire l'analisi del carico di lavoro negli ambienti NAS (NFS e SMB) al fine di rilevare e segnalare in modo proattivo attività anomale che potrebbero indicare un attacco ransomware. A partire da ONTAP 9.17.1, ARP supporta anche i volumi a blocchi, inclusi i volumi SAN contenenti LUN o namespace NVMe, o i volumi NAS contenenti dischi virtuali di hypervisor come VMware. A partire da ONTAP 9.17.1P5, Hyper-V, KVM e OpenStack hypervisor sono anch'essi supportati, e ARP aggiunge una modalità di valutazione per i dischi virtuali contenuti nei volumi NAS (vedere <<Ambienti NAS e tipi di modalità>>). ARP è integrato direttamente in ONTAP, garantendo controllo e coordinamento integrati con le altre funzionalità di ONTAP. ARP opera in real-time, elaborando i dati man mano che vengono scritti o letti dal file system e rilevando e rispondendo rapidamente a potenziali attacchi ransomware.
