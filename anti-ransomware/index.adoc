...
[role="lead"]
A partire da ONTAP 9.10.1, gli amministratori di ONTAP possono abilitare Autonomous Ransomware Protection (ARP) per eseguire l'analisi del carico di lavoro negli ambienti NAS (NFS e SMB) al fine di rilevare e segnalare in modo proattivo attività anomale che potrebbero indicare un attacco ransomware. ARP è integrato direttamente in ONTAP, garantendo controllo e coordinamento integrati con le altre funzionalità di ONTAP. ARP opera in real-time, elaborando i dati man mano che vengono scritti o letti dal file system e rilevando e rispondendo rapidamente a potenziali attacchi ransomware.

ARP crea snapshot bloccati a intervalli regolari, oltre agli snapshot programmati. La conservazione degli snapshot viene regolata dinamicamente: gli snapshot vengono eliminati rapidamente quando non viene rilevata alcuna attività insolita, mentre gli snapshot acquisiti prima di un attacco rilevato vengono conservati più a lungo per consentire l'analisi e il ripristino.

Per informazioni sul supporto del protocollo, l'idoneità del carico di lavoro e i requisiti specifici della versione, consultare link:use-cases-restrictions-concept.html["Casi d'uso e considerazioni relative ad ARP"].

Per ulteriori informazioni sugli snapshot generati da ARP, comprese le modifiche aggiunte dalla versione di ONTAP, vedere <<Snapshot ARP>>.
...
Il supporto ARP è incluso conlink:https://kb.netapp.com/onprem/ontap/os/ONTAP_9.10.1_and_later_licensing_overview["Licenza ONTAP One"^] . Se non si dispone della licenza ONTAP One, sono disponibili altre licenze per l'uso ARP che variano a seconda della versione di ONTAP
...
A partire da ONTAP 9.18.1, ARP è abilitato automaticamente per impostazione predefinita su tutti i nuovi volumi per AFF serie A e AFF serie C, ASA e ASA r2. Questa abilitazione automatica predefinita di ARP non si applica a link:use-cases-restrictions-concept.html#unsupported-configurations["volumi o configurazioni non supportate"]. È necessario <<manual-default-enablement,abilitare ARP manualmente>> sui volumi esistenti. Per dettagli su come l'abilitazione predefinita ha effetto durante l'aggiornamento, vedere link:use-cases-restrictions-concept.html#arp-upgrade-considerations["Considerazioni sull'aggiornamento specifiche per la versione di ARP"].


NOTE: A partire da ONTAP 9.19.1, l'abilitazione predefinita di ARP si applica ai cluster che includono SnapMirror synchronous e SnapMirror active sync relazioni SAN. In ONTAP 9.18.1, l'abilitazione predefinita era stata soppressa per queste configurazioni. Per le relazioni SnapMirror synchronous tra cluster, l'abilitazione predefinita si attiva una volta che sia il cluster di origine che quello di destinazione vengono aggiornati a ONTAP 9.19.1. ARP non è abilitato per impostazione predefinita sui sistemi AFX, nemmeno con il supporto SnapMirror synchronous.

Durante il grace period, puoi link:default-enable-change.html["disattivare l'abilitazione predefinita per i nuovi volumi a livello di cluster utilizzando System Manager o la ONTAP CLI"]. Se non effettui l'opt-out, ARP viene automaticamente abilitato per tutti i nuovi volumi creati dopo la fine del grace period. Se le esigenze cambiano dopo il grace period, hai anche la flessibilità di attivare o disattivare l'abilitazione predefinita in qualsiasi momento.

.Abilitazione manuale predefinita sui nuovi volumi
Se hai disabilitato l'abilitazione automatica predefinita di ARP a livello di cluster, puoi anche scegliere di link:enable-default-task.html["abilitare manualmente ARP per impostazione predefinita su tutti i nuovi volumi"] a livello di SVM. Per ONTAP 9.17.1 e versioni precedenti, questo è l'unico modo per configurare ARP in modo che sia abilitato per impostazione predefinita sui nuovi volumi.

[[manual-default-enablement]]
.Abilitazione ARP su tutti o su specifici volumi esistenti
A partire dalla versione 9.18.1, è possibile abilitare manualmente ARP su tutti i volumi esistenti dal livello di cluster (selezionare *Cluster > Security* e image:icon_kabob.gif["Icona delle opzioni di menu"] nella sezione *Anti-ransomware*, quindi selezionare *Abilita su tutti i volumi esistenti*).

Se preferisci limitare l'abilitazione ARP a un volume specifico, puoi link:enable-task.html["abilitare ARP su base per-volume"].
...
* link:https://docs.netapp.com/us-en/netapp-solutions-dataops/cyber-vault/ontap-cyber-vault-ps-hardening.html#ontap-cli-or-via-automation-script["Indurimento del caveau informatico ONTAP con PowerShell"^]
...
Nelle versioni con il precedente modello ARP, si consiglia un periodo di apprendimento prima di iniziare il monitoraggio attivo.  Per ambienti NAS che supportano<<arp-ai,ARP/IA>> , non c'è un periodo di apprendimento e il monitoraggio attivo inizia immediatamente.
...
Valutazione
 a| 
Viene eseguito un periodo di valutazione da due a quattro settimane per determinare il comportamento di base della crittografia. Il rilevamento e gli avvisi possono verificarsi durante la definizione delle soglie di base. È possibile determinare se il periodo di valutazione è completato eseguendo il `security anti-ransomware volume show` comando e controllando `Block device detection status`.

link:respond-san-entropy-eval-period.html["Scopri il periodo di valutazione di ONTAP ARP/AI per i carichi di lavoro su dispositivi a blocchi"].
 a| 
Volumi NAS che contengono dischi virtuali hypervisor rilevati da ONTAP con ONTAP 9.17.1P5 e versioni successive
...
.Differenze tra le snapshot ARP in base alla versione
* *Modifica le impostazioni di conservazione*: A partire da ONTAP 9.11.1, è possibile link:modify-automatic-snapshot-options-task.html["modificare le impostazioni di conservazione per gli snapshot"].
* *Intervalli e risposta alle anomalie*: A partire da ONTAP 9.17.1, gli snapshot ARP vengono generati a intervalli regolari sia per i volumi NAS che SAN, nonché in risposta alle anomalie rilevate, che è sempre stata la risposta standard. ONTAP antepone un nome allo snapshot ARP per renderlo facilmente identificabile.
* *SnapMirror sincronizzazione sincrona e attiva*: A partire da ONTAP 9.19.1, gli snapshot ARP creati per i volumi che partecipano a relazioni di SnapMirror sincronizzazione sincrona o SnapMirror active sync seguono queste regole aggiuntive:
+
** Gli snapshot ARP vengono creati e conservati sul volume primario.
** Gli snapshot ARP non vengono replicati sul volume secondario come parte della SnapMirror sincronizzazione sincrona o SnapMirror active sync. È necessario link:enable-task.html["riattivare ARP"] sul volume di ripristino dopo il failover.





NOTE: Il ripristino a livello di volume (`volume snapshot restore` su SnapMirror synchronous o SnapMirror active sync volumi SAN potrebbe richiedere che tu sospenda temporaneamente o interrompa la relazione di SnapMirror synchronous o SnapMirror active sync. In molti casi, puoi evitare di interrompere SnapMirror synchronous o SnapMirror active sync utilizzando FlexClone o operazioni di ripristino a livello di file da ARP o altri snapshot sul volume primario.

link:use-cases-restrictions-concept.html#snapmirror["Scopri di più sul supporto per SnapMirror sincrono e SnapMirror active sync"].
...
Con ARP/AI, non c'è<<Scopri le modalità ARP,periodo di apprendimento>> .  ARP/AI è attivo immediatamente dopo l'installazione o l'aggiornamento per i seguenti tipi di volume supportati:

* Volumi NAS FlexVol con ONTAP 9.16.1 e versioni successive
* Volumi NAS FlexGroup con ONTAP 9.18.1 e versioni successive
* Volumi SAN con ONTAP 9.17.1 e versioni successive (attivi immediatamente, anche durante lalink:respond-san-entropy-eval-period.html["periodo di valutazione"] )
