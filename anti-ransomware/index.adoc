...
[role="lead"]
A partire da ONTAP 9.10.1, gli amministratori di ONTAP possono abilitare la protezione autonoma contro i ransomware (ARP) per eseguire l'analisi del carico di lavoro negli ambienti NAS (NFS e SMB) al fine di rilevare e segnalare in modo proattivo attività anomale che potrebbero indicare un attacco ransomware. A partire da ONTAP 9.17.1, ARP supporta anche volumi di dispositivi a blocchi, inclusi volumi SAN contenenti LUN o namespace NVMe, o volumi NAS contenenti dischi virtuali da hypervisor come VMware. A partire da ONTAP 9.17.1P5, sono supportati anche Hyper-V, KVM e OpenStack hypervisor.
