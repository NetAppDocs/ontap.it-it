...
^2^ A partire da ONTAP 9.18.1, il comportamento di abilitazione predefinito di ARP/AI è disponibile per AFF serie A e AFF serie C, ASA e ASA r2. Questo comportamento abilita automaticamente ARP/AI su tutti i nuovi volumi dopo un grace period di 12 ore a seguito di un aggiornamento o immediatamente per le nuove installazioni di ONTAP 9.18.1. Sarà necessario abilitare manualmente ARP su link:enable-task.html["volumi esistenti"].
...
* Dopo l'aggiornamento a ONTAP 9.19.1 RC e l'abilitazione di ARP sui volumi che fanno parte di relazioni SnapMirror sincrone o SnapMirror active sync, la relazione potrebbe trovarsi brevemente in uno stato di non sincronizzazione. Questa condizione è temporanea e la relazione si risincronizzerà automaticamente senza alcun intervento.
