...
NOTE: Se si utilizza il `volume snap restore` comando come descritto di seguito, è necessario rilasciare il SnapLock anti-ransomware prima del ripristino da copie Snapshot precedenti. Se si ripristinano i dati utilizzando Flex Clone, Single file Snap Restore o altri metodi, ciò non è necessario.
