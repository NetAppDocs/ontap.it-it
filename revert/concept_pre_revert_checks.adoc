...
* link:arp-warning-clear-revert.html["Rispondi agli avvisi di protezione autonoma dal ransomware in caso di attività anomale"].




| ONTAP 9.19.1  a| 
* Se hai SVM con configurazioni multipathing attivo-attivo, devi link:delete-active-active-svms.html["elimina gli SVM"].
* Se hai modificato le impostazioni predefinite su un gruppo di criteri QoS creato in ONTAP 9.19.1 o versioni successive, devi link:modify-qos-policy-groups.html["ripristinali"].
* Se hai aggiornato i volumi a ONTAP 9.19.1 e hai aggiunto una seconda chiave di accesso S3 a un utente qualsiasi, devi link:second-s3-access-key.html["rimuovi queste chiavi"].




| ONTAP 9.18.1  a| 
* Se l'abilitazione automatica è stata impostata per ARP/AI come parte di un aggiornamento ONTAP 9.18.1, dovrai link:anti-ransomware-disable-auto-enable.html["disattivarlo"].




| ONTAP 9.17.1  a| 
* Se hai abilitato la funzionalità ONTAP ARP/AI per SAN, link:anti-ransomware-disable-san.html["disattivarlo"].
