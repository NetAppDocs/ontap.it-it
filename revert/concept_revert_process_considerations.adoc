...
+
link:https://docs.netapp.com/us-en/ontap-afx/administer/revert-ontap.html["Sistemi di stoccaggio AFX"^]non supportano la reversione.
...
IMPORTANT: Se hai ripristinato alcuni nodi, ma non tutti, non tentare di aggiornare il cluster alla versione originale.
...
* Nei sistemi MetroCluster IP che utilizzano switch conformi a MetroCluster ma non convalidati MetroCluster, il passaggio da ONTAP 9.7 a 9.6 è dirompente in quanto non è supportato alcun sistema che utilizza ONTAP 9.6 e versioni precedenti.
...
+
Se si tenta di ripristinare una versione ONTAP che non supporta la crittografia del volume radice SVM, il sistema risponderà con un avviso e bloccherà il ripristino.

* Se hai abilitato NFS su TLS in ONTAP 9.19.1 o versioni successive, devi disabilitare tutte le istanze di configurazione NFS su TLS prima di poter tornare a una versione precedente di ONTAP. NFS su TLS è supportato solo a partire da ONTAP 9.19.1.
