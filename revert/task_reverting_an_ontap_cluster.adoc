...
+
IL `-version` l'opzione si riferisce alla versione ONTAP a cui si desidera tornare. Sono richiesti solo i primi due valori della versione ONTAP . Ad esempio, se si sta ripristinando da 9.14.1 a 9.13.1, il valore corretto di `-version` l'opzione è 9.13.
...
. Dopo aver ripristinato entrambi i nodi, riattiva l'HA per il cluster:
+
[source, cli]
----
...
NOTE: La riattivazione dell'HA del cluster potrebbe far sì che l' `storage failover auto-giveback`impostazione torni a true.
...
----
. Verifica l' `storage failover auto-giveback`impostazione su entrambi i nodi:
+
[source, cli]
----
storage failover show -node <nodename>
----
+
Se per il tuo ambiente è necessaria un'impostazione diversa, usa il comando  `storage failover modify -node <nodename> -auto-giveback <true|false>` per modificarla.
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp Knowledge Base: perché il ripristino automatico è abilitato dopo aver riattivato l'impostazione di failover dello storage"^]
