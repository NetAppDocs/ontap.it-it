...
Se si torna da ONTAP 9.12.1 o versione successiva a ONTAP 9.12.0 o versione precedente, è necessario link:../nvme/disable-secure-authentication-nvme-task.html["disattiva l'autenticazione in banda"] prima di tornare indietro.  Se l'autenticazione in banda mediante DH-HMAC-CHAP non è disattivata, l'operazione di revert avrà esito negativo.
