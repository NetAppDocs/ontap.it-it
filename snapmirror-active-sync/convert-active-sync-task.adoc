...
---
= Converti una relazione SAN ONTAP SnapMirror esistente in SnapMirror active sync per relazione SAN
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Se hai configurato la protezione SnapMirror, puoi convertire una relazione SAN in SnapMirror active sync per SAN. A partire da ONTAP 9.15.1, puoi convertire la relazione per utilizzare la protezione simmetrica active/active.

Per informazioni sulla conversione di una relazione SVM-DR in una relazione SnapMirror active sync per NAS, vedi link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["Converti una relazione SVM DR in una relazione di sincronizzazione attiva SnapMirror"].
