...
---
= Collegamento tra il sito B e il mediatore ONTAP inattivo e il sito A inattivo
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Annullare la mappatura di tutti i LUN appartenenti al gruppo di coerenza dal sito B. Questa operazione non riuscirà, quindi è necessario prima modificare l'igroup per rimuovere la SVM del peer di replicazione e quindi eliminare la mappa LUN.
