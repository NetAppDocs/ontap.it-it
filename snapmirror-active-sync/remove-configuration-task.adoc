...
== Rimuovi una configurazione SAN asimmetrica

* Prima di eliminare la relazione di sincronizzazione attiva SAN di SnapMirror, tutte le LUN nel cluster di destinazione devono essere smappate.
...
. Nella finestra *Elimina configurazione di replica*, seleziona *Rilascia gli snapshot di base del volume di origine.* e seleziona *Elimina*.
...
== Rimuovi una configurazione NAS
...
. Nel cluster di origine, passare a *Protezione > Replica*.
. Individua la relazione che desideri rimuovere, seleziona image:icon_kabob.gif["Icona delle opzioni di menu"] e seleziona *Elimina*.
. Nella finestra *Elimina configurazione di replica*, seleziona *Rilascia gli snapshot di base della storage VM di origine* e seleziona *Elimina*.
...
----
+
L'SVM e i relativi volumi vengono rimossi dal cluster secondario.
