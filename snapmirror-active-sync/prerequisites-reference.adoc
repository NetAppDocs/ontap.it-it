...
A-Series e C-Series
 a| 
Automated Failover Duplex (simmetrico attivo/attivo), Automated Failover (asimmetrico attivo/passivo)
 a| 
...
NOTE: Se i sistemi di archiviazione ONTAP sono stati acquistati prima di giugno 2019, vedere link:https://mysupport.netapp.com/site/systems/master-license-keys["Chiavi di licenza master ONTAP"^] per ottenere la licenza sincrona SnapMirror richiesta.
...
* Nelle release precedenti a ONTAP 9.15.1, le relazioni di SnapMirror active sync non sono supportate sui volumi di destinazione in lettura-scrittura (volumi convertiti in lettura-scrittura da DP in un attivo-passivo asimmetrico). Prima di poter utilizzare un volume in lettura-scrittura, è necessario convertirlo in un volume DP creando una relazione di SnapMirror a livello di volume (asincrona o sincrona) e poi eliminando la relazione. Per dettagli, vedere link:convert-active-sync-task.html["Converti relazioni di SnapMirror esistenti in sincronizzazione attiva SnapMirror"].
