...
* ONTAP Mediator 1.12.1 o versioni successive è installato su un host con una CPU a 2 core.
...
IMPORTANT: Se si utilizza SnapMirror active sync e ONTAP Mediator o ONTAP Cloud Mediator con ONTAP 9.17.1, è necessario consultare la sezione *Problemi noti e limitazioni* nel link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["Note di rilascio di ONTAP"^] per informazioni importanti su queste configurazioni.
...
Quando si utilizza ONTAP Mediator per monitorare una SnapMirror active sync relationship, ONTAP Mediator non può essere inizializzato in ONTAP senza un certificato autofirmato o di un'autorità di certificazione (CA) valido. Si aggiunge un certificato valido all'archivio certificati per i cluster peered.
...
-----END CERTIFICATE-----
----
. Installare i certificati CA radice e intermedi sul cluster primario o secondario:
+
...
+
Inserisci il  `root.crt` quando richiesto, come mostrato nell'esempio seguente:
...
----
. Ripetere il passaggio precedente per installare i certificati CA radice e intermedi sul cluster peered.
. Visualizza i certificati CA autofirmati installati utilizzando il nome generato del certificato:
+
