...
Quando si utilizza ONTAP Mediator per monitorare una SnapMirror active sync relationship, ONTAP Mediator non può essere inizializzato in ONTAP senza un certificato autofirmato o di un'autorità di certificazione (CA) valido. Si aggiunge un certificato valido all'archivio certificati per i cluster peered. Quando si utilizza ONTAP Mediator per monitorare i sistemi IP MetroCluster, HTTPS non viene utilizzato dopo la configurazione iniziale; pertanto, i certificati non sono richiesti.
...
-----END CERTIFICATE-----
----
. Aggiungere il certificato CA di ONTAP Mediator a un cluster ONTAP. Quando richiesto, inserire il certificato CA ottenuto da ONTAP Mediator. Ripetere i passaggi su tutti i cluster peer:
+
