...
Quando si utilizza ONTAP Mediator per monitorare una relazione di sincronizzazione attiva SnapMirror, ONTAP Mediator non può essere inizializzato in ONTAP senza un certificato autofirmato o di un'autorità di certificazione (CA) valido. Si aggiunge un certificato valido all'archivio certificati per i cluster peer.
