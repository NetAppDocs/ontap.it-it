...
* Capacità di ospitare applicazioni critiche in maniera alternativa dal sito primario e secondario.
...
* Gruppi di coerenza (che è l'unità di failover); per ogni gruppo di coerenza vengono fornite le seguenti informazioni:
+
...
La sincronizzazione attiva di SnapMirror utilizza un principio primario che privilegia la copia primaria per l'i/o nel caso di una partizione di rete.
...
Quando si crea una protezione con _symmetric Active/Active_, il processo dipende dalla configurazione:
