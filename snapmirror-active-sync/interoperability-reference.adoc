...
Per ulteriori informazioni sulla supportabilità delle configurazioni NAS, vedi link:protect-nas-task.html["Proteggi con ONTAP SnapMirror active sync per NAS"].
...
* AIX
+
** A partire da ONTAP 9.19.1, il Transparent Application Failover (TAF) è supportato con configurazioni simmetriche active/active. Non è richiesto alcun PVR.
** A partire da ONTAP 9.11.1 fino a ONTAP 9.18.1, AIX è supportato solo da PVR per il failover automatico con configurazioni simmetriche attivo/attivo.
...
A partire da ONTAP 9.19.1, il failover trasparente delle applicazioni per AIX è supportato con SnapMirror active sync in configurazioni simmetriche attivo/attivo. È possibile utilizzare System Manager o la ONTAP CLI per creare un LUN AIX. Per garantire il failover trasparente, quando si utilizza la ONTAP CLI, è necessario utilizzare l'opzione access-mode local con il comando lun create o lun modify. Per impostazione predefinita, la modalità di accesso è impostata su global. Quando si utilizza System Manager, scegliere AIX come host e local come modalità di accesso. Per ulteriori informazioni sulla creazione di un LUN, consultare link:../san-admin/provision-storage.html["Eseguire il provisioning dello storage SAN"]. Per ulteriori informazioni sul supporto per AIX, consultare l'articolo della Knowledge Base di NetApp: link:https://kb.netapp.com/on-prem/ontap/DP/SnapMirror/SnapMirror-KBs/SnapMirror_active_sync_support_for_AIX%3A_behavior_by_ONTAP_release_and_migration_guidance["SnapMirror supporto per la sincronizzazione attiva su AIX: comportamento per release ONTAP e guida alla migrazione"^].

Se si utilizza ONTAP 9.11.1 fino a ONTAP 9.18.1, AIX è supportato con SnapMirror active sync tramite la richiesta standard di Feature Policy Variance Request (FPVR), con l'accordo che vengano comprese le seguenti disposizioni:
...
Un evento di failover automatico non pianificato (AUFO) sul cluster primario isolato può essere causato da un doppio guasto quando viene persa la connessione tra il cluster primario e quello secondario e viene persa anche la connessione tra il cluster primario e il mediatore. Questo è considerato un evento raro, a differenza di altri eventi AUFO.
...
Per garantire che le applicazioni client Solaris non interrompano le attività quando si verifica un passaggio di failover del sito non pianificato in un ambiente SnapMirror ActiveSync, modificare le impostazioni predefinite del sistema operativo Solaris. Per configurare Solaris con le impostazioni consigliate, vederelink:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/Solaris_Host_support_recommended_settings_in_SnapMirror_Business_Continuity_(SM-BC)_configuration["Base di conoscenza NetApp : impostazioni consigliate per il supporto host Solaris in SnapMirror ActiveSync"^] .
...
A partire da ONTAP 9.19.1, ARP/AI supporta i volumi primari che partecipano a relazioni SAN di sincronizzazione attiva SnapMirror. Per ulteriori informazioni, vedi link:../anti-ransomware/use-cases-restrictions-concept.html#interoperability-and-vm-considerations["Casi di utilizzo e considerazioni sulla protezione ransomware autonoma"].
...
SnapMirror active sync per SAN supporta xref:../data-protection/supported-deployment-config-concept.html[configurazioni fan-out] con la `MirrorAllSnapshots` policy e, a partire da ONTAP 9.11.1, la `MirrorAndVault` policy. Le configurazioni fan-out non sono supportate in SnapMirror active sync con la `XDPDefault` policy.

A partire da ONTAP 9.15.1, SnapMirror active sync supporta la riconfigurazione automatica nel ramo fan-out dopo un evento di failover. Se il failover dal sito primario a quello secondario ha avuto successo, la riconfigurazione della relazione fan-out viene avviata durante il successivo aggiornamento pianificato del ramo fan-out asincrono. Non viene attivata immediatamente dopo il failover della sincronizzazione attiva di SnapMirror. Il ramo fan-out asincrono può essere una relazione di gruppo di coerenza in una configurazione SAN o una relazione di volume indipendente. La riconfigurazione funziona in entrambi i casi. La riconfigurazione viene attivata sia da un failover pianificato che non pianificato. La riconfigurazione avviene anche in caso di failback al sito primario.
...
A partire da ONTAP 9.12.1, link:../data-protection/restore-part-file-snapshot-task.html["Ripristino parziale del LUN"] è supportata per i volumi SnapMirror active sync. Puoi ripristinare i dati da snapshot creati dall'applicazione che sono stati replicati tra il volume di origine SnapMirror active sync e i volumi di destinazione (snapshot). Il ripristino parziale di LUN o file può essere necessario se devi ripristinare un database su un host che memorizza più database sullo stesso LUN. Per usare questa funzionalità devi conoscere l'offset iniziale in byte dei dati e il numero di byte.
...
Gli snapshot a prova di manomissione non sono supportati con SnapMirror active sync.
