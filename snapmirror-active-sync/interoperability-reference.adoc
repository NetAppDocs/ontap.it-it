...
A partire da ONTAP 9.15.1, SnapMirror active sync supporta la riconfigurazione automatica nel ramo fan-out dopo un evento di failover. Se il failover dal sito primario al sito secondario ha avuto successo, il sito terziario viene automaticamente riconfigurato per considerare il sito secondario come origine. Il ramo fan-out asincrono può essere una relazione di gruppo di coerenza in una configurazione SAN o una relazione di volume indipendente. La riconfigurazione funziona in entrambi i casi. La riconfigurazione viene attivata sia da un failover pianificato che non pianificato. La riconfigurazione avviene anche in caso di failback al sito primario.
