...
---
= Proteggi con ONTAP SnapMirror active sync per SAN
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Configurare la protezione asimmetrica per le configurazioni SAN usando SnapMirror active sync significa selezionare le LUN sul cluster di origine ONTAP e aggiungerle a un gruppo di coerenza.
...
* Il cluster di origine e il cluster di destinazione non possono far parte di una configurazione MetroCluster.
...
Puoi stabilire una protezione simmetrica per le configurazioni SAN utilizzando System Manager o la ONTAP CLI. In entrambe le interfacce, ci sono passaggi diversi per xref:index.html#key-concepts[configurazioni uniformi e non uniformi].
...
. Sul sito principale, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["creare un gruppo di coerenza utilizzando nuove LUN"].
+
...
.System Manager
--
. Sul sito principale, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["creare un gruppo di coerenza utilizzando nuovi volumi o namespace NVMe"].
