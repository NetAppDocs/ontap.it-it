...
Potresti voler eseguire un failover pianificato per testare l'integrità della configurazione di disaster recovery o per effettuare manutenzione sul cluster primario.
...
. Per monitorare lo stato del failover, usa il comando  `snapmirror failover show` nella CLI di ONTAP.
...
+
Esempio di failover di una configurazione SAN:

+
destination::>snapmirror failover start -destination-path vs1_dst:/cg/cg_dst

+
Esempio di failover di una configurazione NAS:

+
destination::>snapmirror failover start -destination-path vs_b:

. Dalla destinazione, monitora l'avanzamento del failover:
+
