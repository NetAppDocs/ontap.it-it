...
== Limita l'insieme dei domini Relying Party (RP) consentiti

A partire da ONTAP 9.19.1, devi usare il parametro -rp-domains nel comando delle impostazioni globali WebAuthn. Questo permette di limitare esplicitamente l'insieme dei domini Relying Party (RP) consentiti durante WebAuthn (FIDO2) multi-factor authentication in System Manager.

Potrai accedere all'autenticazione a più fattori (MFA) solo dai domini RP consentiti.


NOTE: Se stai eseguendo l'aggiornamento a ONTAP 9.19.1 da ONTAP 9.16.1 a 9.18.1, consulta link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688["CONTAP-597688"] per ulteriori informazioni.

[role="tabbed-block"]
====
.System Manager
--
. Seleziona *Cluster > Impostazioni > Multifactor authentication (MFA)*.
. Aggiungi, elimina o modifica i domini RP consentiti.


--
.CLI
--
. Modifica i domini rp consentiti. Imposta -rp-domains sul FQDN completo del cluster:
+
[source, console]
----
...
----
+
Scopri di più su `security webauthn modify` nella link:https://docs.netapp.com/us-en/ontap-cli/security-webauthn-modify.html#parameters["Riferimento al comando ONTAP"^].
