...
summary: 'Esistono due tipi di configurazione di base per FPolicy. Una configurazione utilizza server FPolicy esterni per elaborare e gestire le notifiche. L"altra configurazione non utilizza server FPolicy esterni. Invece, utilizza il server FPolicy nativo interno di ONTAP per il blocco semplice dei file in base alle estensioni.' 
---
