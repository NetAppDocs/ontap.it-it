...
---
= Requisiti e best practice per la configurazione di ONTAP FPolicy
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Se il volume dell'archivio persistente si sposta da un nodo all'altro nello stesso SVM, anche le notifiche che devono ancora essere elaborate vengono spostate sul nuovo nodo.  Devi rieseguire il `fpolicy persistent-store create` comando su entrambi i nodi dopo che il volume è stato spostato per garantire che le notifiche in sospeso vengano recapitate al server esterno.

Scopri di più su  `fpolicy persistent-store create` nel link:https://docs.netapp.com/us-en/ontap-cli/vserver-fpolicy-persistent-store-create.html["Riferimento al comando ONTAP"^] .
