...
Il gestore di chiavi integrato memorizza e gestisce le chiavi in un database sicuro interno al cluster. Il suo ambito è il cluster. Un gestore di chiavi esterno memorizza e gestisce le chiavi al di fuori del cluster. Il suo ambito può essere il cluster o la storage VM.

.A proposito di questa attività
È necessario utilizzare l'interfaccia a riga di comando ONTAP (CLI) per configurare i gestori di chiavi esterni per le configurazioni MetroCluster. Scopri di più su link:configure-external-key-management-concept.html["Configurare la gestione delle chiavi esterne ONTAP usando la CLI"].

È possibile utilizzare uno o più gestori di chiavi esterni. Si applicano le seguenti condizioni:
