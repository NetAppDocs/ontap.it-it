...
È necessario utilizzare l'interfaccia a riga di comando ONTAP (CLI) per configurare i gestori di chiavi esterni per le configurazioni MetroCluster. Scopri di più su link:configure-external-key-management-concept.html["${post_edited_translations.segment}"].
