...
Ulteriori informazioni su `security key-manager onboard show-backup` nella link:https://docs.netapp.com/us-en/ontap-cli/security-key-manager-onboard-show-backup.html["Riferimento al comando ONTAP"^].

+ il seguente comando 9.6 visualizza le informazioni di backup per la gestione delle chiavi per `cluster1`:
