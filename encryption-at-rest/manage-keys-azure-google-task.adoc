...
. Prima di iniziare, devi ottenere le credenziali per l'autenticazione dal tuo account Azure, ovvero un client secret o un certificato. Devi anche assicurarti che tutti i nodi del cluster siano integri. Puoi verificarlo con il comando `cluster show`. Scopri di più su `cluster show` nella link:https://docs.netapp.com/us-en/ontap-cli/cluster-show.html["Riferimento al comando ONTAP"^].
. Impostare il livello di privilegi su avanzato
`set -priv advanced`
. Abilita AKV sull'SVM A partire da ONTAP 9.19.1, usa: 
`security key-manager external azure create-config -config-name _config_name_ -client-id _client_id_ -tenant-id _tenant_id_ -name URL -key-id _key_id_ -authentication-method {certificate|client-secret} -vserver _vserver_name_` seguito da: 
`security key-manager keystore enable -vserver _vserver_name_ -keystore-type AKV -config-name _config_name_` Per ONTAP 9.18.1 e versioni precedenti, usa: 
`security key-manager external azure enable -client-id _client_id_ -tenant-id _tenant_id_ -name _name_ -key-id _key_id_ -authentication-method {certificate|client-secret} -vserver _vserver_name_` Quando richiesto, inserisci il certificato client o il client secret dal tuo account Azure.
...
`security key-manager external gcp show vserver _svm_name_` Lo stato di `kms_wrapped_key_status` sarà `"UNKNOWN"` se non sono stati creati volumi crittografati. Se la raggiungibilità del servizio non è corretta, stabilire la connettività al servizio di gestione delle chiavi GCP tramite dati SVM LIF.
+
