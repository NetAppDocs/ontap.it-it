...
È possibile utilizzare Onboard Key Manager per proteggere le chiavi utilizzate dal cluster per accedere ai dati crittografati.  Abilita Onboard Key Manager su ogni cluster che accede a volumi crittografati o dischi auto-crittografanti.
...
.Prima di iniziare
* Se si utilizza NSE con un server di gestione delle chiavi esterno (KMIP), eliminare il database del gestore delle chiavi esterno.
+
...
.Al termine
ONTAP esegue automaticamente il backup delle informazioni di gestione delle chiavi nel database replicato (RDB) per il cluster.
