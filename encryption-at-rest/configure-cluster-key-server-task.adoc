...
. Modificare il server delle chiavi primarie per aggiungere server delle chiavi secondarie. IL `-secondary-key-servers` Il parametro accetta un elenco separato da virgole di un massimo di tre server chiave. Non includere un numero di porta per i server chiave secondari.
...
--
. Modificare il server delle chiavi primarie per aggiungere server delle chiavi secondarie. IL `-secondary-key-servers` Il parametro accetta un elenco separato da virgole di un massimo di tre server chiave. Non includere un numero di porta per i server chiave secondari.
`security key-manager external modify-server -vserver _svm_name_ -key-servers _primary_key_server_ -secondary-key-servers _list_of_key_servers_` Per ulteriori informazioni sui server di chiavi secondarie, vedere <<mod-secondary>>.
...
I server delle chiavi secondarie sono gestiti con `-secondary-key-servers` parametro del `security key-manager external modify-server` comando. IL `-secondary-key-servers` il parametro accetta un elenco separato da virgole. L'ordine specificato dei server delle chiavi secondarie nell'elenco determina la sequenza di accesso per i server delle chiavi secondarie. L'ordine di accesso può essere modificato eseguendo il comando `security key-manager external modify-server` con i server delle chiavi secondarie inseriti in una sequenza diversa. Non includere un numero di porta per i server chiave secondari.
