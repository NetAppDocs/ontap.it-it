...
[role="lead"]
A partire da ONTAP 9,7, gli aggregati e i volumi appena creati sono crittografati per impostazione predefinita, quando si dispone di link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/install-license-task.html["Licenza VE"] e gestione della chiave integrata o esterna. A partire da ONTAP 9.6, è possibile utilizzare la crittografia a livello di aggregato per assegnare le chiavi all'aggregato contenente per i volumi da crittografare.
