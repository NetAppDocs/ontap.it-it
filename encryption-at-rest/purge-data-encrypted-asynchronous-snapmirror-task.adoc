...
+
.. Crea un'istantanea sul volume di origine nella relazione asincrona SnapMirror:
+
`volume snapshot create -snapshot snapshot_name -vserver _SVM_name_ -volume _volume_name_`

.. Aggiorna la relazione di SnapMirror per spostare l'istantanea di base avanti verso la destinazione:
+
