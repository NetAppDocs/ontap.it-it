...
---
= Esegui un aggiornamento ONTAP automatizzato e senza interruzioni
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Quando esegui un aggiornamento automatico non interruttivo (ANDU), ONTAP installa automaticamente l'immagine ONTAP di destinazione su ciascun nodo, verifica che il cluster possa essere aggiornato correttamente e poi esegue xref:concept_upgrade_methods.html[batch o rolling upgrade] in background in base al numero di nodi nel cluster.
...
IMPORTANT: La modifica dell'impostazione della  `storage failover modify-auto-giveback` opzione di comando prima dell'inizio di un ANDU non ha alcun impatto sul processo di aggiornamento. Il processo ANDU ignora qualsiasi valore preimpostato per questa opzione durante il takeover/giveback richiesto per l'aggiornamento. Ad esempio, impostare  `-autogiveback` su false prima di iniziare ANDU non interrompe l'aggiornamento automatico prima del giveback. Scopri di più su  `storage failover modify-auto-giveback` nella link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["Riferimento al comando ONTAP"^].
...
* Opzionalmente, link:../update/pre-stage-system-firmware.html["Pre-configura il firmware di sistema per un aggiornamento con un solo riavvio"].
+
A partire da ONTAP 9.17.1, se sul dispositivo di avvio di un nodo è già presente un firmware BIOS o LOADER più recente prima di eseguire un ANDU, il processo di aggiornamento conserva il firmware più recente invece di sovrascriverlo con la versione inclusa nell'immagine ONTAP. Questo ti permette di completare sia l'aggiornamento del software ONTAP che l'aggiornamento del firmware in un unico ciclo di riavvio.
...
+
Per FTP anonimo, è necessario specificare l'URL nel formato \ftp://anonymous@ftpserver.
...
|===
.. Seleziona *Convalida* per eseguire i controlli di convalida preliminari all'aggiornamento.
+
...
+
Viene visualizzato un elenco completo dei controlli preliminari automatici per l'upgrade, insieme a eventuali errori o avvisi che devono essere risolti prima di avviare il processo di upgrade. È necessario risolvere tutti gli errori prima di procedere con l'upgrade. È best practice risolvere anche gli avvisi.
...
Se hai preconfigurato il firmware, usa il comando  `system node firmware bootmedia show` per verificare che la versione del firmware prevista sia attiva su ciascun nodo.
