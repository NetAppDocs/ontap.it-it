...
---
= Verifica della configurazione di failover LIF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Prima di eseguire un aggiornamento, è necessario verificare che i criteri di failover e i gruppi di failover siano configurati correttamente.
...
Se nel cluster sono presenti meno di 8 nodi, l'aggiornamento automatico viene eseguito utilizzando il metodo a rotazione. Il metodo di aggiornamento in sequenza prevede l'avvio di un'operazione di failover su ciascun nodo di una coppia ha, l'aggiornamento del nodo "guasto", l'avvio del giveback e la ripetizione del processo per ogni coppia ha nel cluster. Se viene utilizzato il metodo rolling, i LIF vengono migrati nel nodo di destinazione del failover come definito dal criterio di failover LIF.
...
----
+
Il campo *failover targets* (destinazioni di failover) mostra un elenco con priorità di destinazioni di failover per ciascun LIF. Ad esempio, se lif0 esegue il failover dalla porta home (e0b su node0), tenta per primi di eseguire il failover sulla porta e0c su node0. Se lif0 non riesce a eseguire il failover su e0c, tenta di eseguire il failover sulla porta e0d su node0 e così via.

. Se il criterio di failover è impostato su Disabled (disattivato) per le LIF diverse DALLE LIF SAN, utilizzare il comando di modifica dell'interfaccia di rete per abilitare il failover.
