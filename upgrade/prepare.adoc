...
---
= Prepararsi manualmente per un aggiornamento del software ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Se si sceglie di preparare manualmente l'upgrade, è necessario completare diverse attività per verificare la presenza di eventuali problemi o blocchi noti e assicurarsi che il cluster soddisfi i requisiti necessari per l'upgrade. Ad esempio, se la modalità SSL FIPs è abilitata sul cluster e gli account amministratore utilizzano chiavi pubbliche SSH per l'autenticazione, è necessario verificare che l'algoritmo della chiave host sia supportato nella release di ONTAP di destinazione.


NOTE: La preparazione manuale non è necessaria quando si utilizza Upgrade Advisor in Active IQ Digital Advisor o Upgrade Health Checker. Entrambi gli strumenti forniscono informazioni che aiutano a pianificare l'upgrade, identificare blocchi e avvisi e ridurre al minimo incertezze e rischi. Scopri di più su link:create-upgrade-plan.html["prepararsi per un upgrade con Upgrade Advisor o Upgrade Health Checker"].

.Fasi
. link:choose-target-version.html["Scegli la tua versione ONTAP di destinazione"].
. Consulta le sezioni _Avvertenze sull'aggiornamento_ e _Problemi noti e limitazioni_ nella link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["Note sulla versione di ONTAP 9"^] per la versione di destinazione.
+
