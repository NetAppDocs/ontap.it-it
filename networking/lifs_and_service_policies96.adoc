...
summary: Assegnare le service policy ai LIF per specificare il traffico supportato. 
---
= Politiche di servizio ONTAP LIF
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
IMPORTANT: I protocolli FCP e NVMe/FCP attualmente non richiedono una service-policy.
...
NOTE: Le applicazioni in una LIF con una politica di servizio vuota potrebbero comportarsi in modo imprevisto.
