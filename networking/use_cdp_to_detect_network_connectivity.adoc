...
summary: In un data center, è possibile utilizzare i protocolli di rilevamento dei vicini per visualizzare la connettività di rete tra una coppia di sistemi fisici o virtuali e le relative interfacce di rete. ONTAP supporta due protocolli di rilevamento dei vicini, Cisco Discovery Protocol e Link Layer Discovery Protocol. 
---
...
CDP deve essere abilitato anche su tutti gli switch e router Ethernet prima che possano essere visualizzate le informazioni sui dispositivi vicini.
...
NOTE: A volte, quando i LIF vengono modificati sul nodo, le informazioni CDP non vengono aggiornate sul dispositivo ricevente (ad esempio, uno switch Ethernet). Se riscontri questo problema, dovresti configurare l'interfaccia di rete del nodo prima nello stato "down" e poi nello stato "up".
...
Diversi switch dei fornitori non supportano CDP. Vedi illink:https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["Knowledge Base NetApp : la scoperta dei dispositivi ONTAP mostra i nodi anziché lo switch"^] per ulteriori dettagli.
