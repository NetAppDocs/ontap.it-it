...
summary: A partire da ONTAP 9.8, è possibile utilizzare la sicurezza IP per proteggere il traffico di rete. IPsec è una delle diverse opzioni di crittografia dei dati in transito o in volo disponibili con ONTAP. È necessario prepararsi a configurare IPsec prima di utilizzarlo in un ambiente di produzione. 
---
= Scopri la sicurezza IP sulla rete ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Se usi ONTAP 9.16.1 o versioni successive, hai la possibilità di delegare alcune operazioni ad alta intensità computazionale, come la crittografia e i controlli di integrità, a una scheda di interfaccia di rete (NIC) installata sul nodo di storage. L'impatto sulle prestazioni di throughput di rete per le operazioni di crittografia IPsec delegate alla scheda NIC è di circa il 5% o meno rispetto alle operazioni in chiaro senza crittografia IPsec abilitata. Questo può migliorare notevolmente le prestazioni e il throughput del traffico di rete protetto da IPsec.
...
* X50131A (Modulo I/O, 2PT, 100/200GbE)
* X50131B (Modulo I/O, 2PT, 200/400GbE,B)
* X60132A (controller Ethernet 4P, 10G/25g)


ONTAP 9.17.1 aggiunge il supporto per le seguenti schede Ethernet:

* X50135A (Modulo I/O, 2PT, 100GbE, Crittografia)
* X60135A (controller Ethernet 2p, 40G/100G)


Le schede X50131A, X50131B e X50135A sono supportate sulle seguenti piattaforme:
