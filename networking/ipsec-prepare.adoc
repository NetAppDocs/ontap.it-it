...
|===
Per ulteriori informazioni sull'API REST ONTAP, https://docs.netapp.com/us-en/ontap-automation/whats-new.html["Novità dell'API REST di ONTAP"^] visitare il sito della documentazione relativa all'automazione ONTAP. È inoltre necessario consultare la documentazione di ONTAP https://docs.netapp.com/us-en/ontap-automation/reference/api_reference.html["Riferimento API"^] per informazioni dettagliate sugli endpoint IPSec.
