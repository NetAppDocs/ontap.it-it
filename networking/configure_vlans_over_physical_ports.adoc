...
summary: Le VLAN forniscono una segmentazione logica delle reti 
---
...
[role="lead"]
Le VLAN forniscono una segmentazione logica delle reti creando domini di trasmissione separati definiti sulla base della porta dello switch rispetto ai domini di trasmissione tradizionali, definiti sui confini fisici.
...
In ONTAP 9,13 e 9,14, le porte non utilizzate e non configurate su VLAN native vengono contrassegnate come degradate. Ciò consente di identificare le porte non utilizzate e non indica un'interruzione. Le VLAN native consentono il traffico non tagged sulla porta base ifgrp, come le trasmissioni ONTAP CFM. Configurare VLAN native sullo switch per impedire il blocco del traffico non tagged.
