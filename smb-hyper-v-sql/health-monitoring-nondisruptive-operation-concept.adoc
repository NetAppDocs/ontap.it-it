...
summary: 'Il monitoraggio dello stato di salute fornisce informazioni sullo stato di salute del sistema nell"intero cluster. L"health monitor monitora Hyper-V e SQL Server su configurazioni SMB per garantire operazioni senza interruzioni per gli application server. Se lo stato è degradato, è possibile visualizzare i dettagli del problema, inclusi la causa probabile e le azioni di ripristino consigliate.' 
---
