...
summary: È possibile utilizzare condivisioni file SMB 3.0 e versioni successive a disponibilità continua per archiviare file di macchine virtuali Hyper-V o database di sistema e database utente di SQL Server su volumi residenti in SVM, garantendo al contempo operazioni non interruttive sia per eventi pianificati che non pianificati. 
---
...
* NetApp Backup and Recovery protegge le macchine virtuali Hyper-V autonome, le istanze del cluster FCI e le macchine virtuali fornite da System Center Virtual Machine Manager (SCVMM) ospitate su condivisioni CIFS.
+
Per informazioni sulla protezione dei carichi di lavoro Hyper-V con NetApp Backup and Recovery, vedi  https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-hyperv-protect-overview.html["Panoramica sulla protezione dei carichi di lavoro Hyper-V"].
