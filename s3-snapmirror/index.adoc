...
S3 SnapMirror supporta le relazioni fan-out e cascata. Per una panoramica, consulta fan-out e implementazioni di protezione dei dati a cascata.

S3 SnapMirror non supporta le implementazioni ffan-in (relazioni di data Protection tra più bucket di origine e un singolo bucket di destinazione). S3 SnapMirror può supportare più mirror bucket da più cluster a un singolo cluster secondario, ma ogni bucket di origine deve avere un proprio bucket di destinazione sul cluster secondario.
