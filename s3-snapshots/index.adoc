...
Gli snapshot S3 catturano solo le versioni correnti degli oggetti nei bucket S3. Le versioni non correnti dei bucket versionati non vengono catturate negli snapshot S3. Inoltre, i tag degli oggetti point-in-time non vengono catturati negli snapshot se i tag degli oggetti vengono modificati dopo che gli snapshot sono stati creati.
...
Dovresti essere consapevole delle seguenti esclusioni e scenari relativi alle funzionalità:
...
* Gli snapshot S3 non sono consigliati sui bucket utilizzati come livello di capacità FabricPool locale o remoto.
