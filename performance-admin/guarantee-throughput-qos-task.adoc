...
summary: 'È possibile utilizzare la qualità del servizio di storage per garantire che le prestazioni dei carichi di lavoro critici non vengano compromesse da carichi di lavoro concorrenti. È possibile impostare un limite massimo di throughput per un carico di lavoro concorrente al fine di limitarne l"impatto sulle risorse di sistema, oppure impostare un limite minimo di throughput per un carico di lavoro critico, assicurandosi che soddisfi i target minimi di throughput, indipendentemente dalla domanda da parte di carichi di lavoro concorrenti. È persino possibile impostare un limite massimo e un limite minimo per lo stesso carico di lavoro.' 
---
= Scopri la garanzia di throughput QoS di ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
È possibile utilizzare la qualità del servizio (QoS) di archiviazione per garantire che le prestazioni dei carichi di lavoro critici non vengano degradate da carichi di lavoro concorrenti. È possibile impostare un throughput  per un carico di lavoro concorrente al fine di limitarne l'impatto sulle risorse di sistema, oppure impostare un throughput  per un carico di lavoro critico, assicurandosi che soddisfi i target minimi di throughput, indipendentemente dalla domanda da parte di carichi di lavoro concorrenti. È possibile impostare un throughput limite massimo e un throughput limite minimo per lo stesso carico di lavoro.

I gruppi di criteri QoS adattivi e non adattivi vengono abilitati automaticamente per aumentare temporaneamente del 50% il throughput massimo di un carico di lavoro per una durata di 1 secondo, in risposta a richieste a breve termine di prestazioni superiori. Ad esempio, si supponga di avviare un database o di avviare più macchine virtuali (VM). Questi carichi di lavoro potrebbero generare un picco nel consumo di risorse, rallentando altri carichi di lavoro. L'aumento automatico e temporaneo del throughput consente a ONTAP di allocare le risorse a breve termine necessarie per inizializzare il database o avviare le macchine virtuali senza il vostro intervento e senza che dobbiate sostenere il costo di un aumento permanente delle risorse di sistema per supportare tali attività. A partire da ONTAP 9.19.1, è possibile utilizzare le  `burst-percent`,  `burst-duration` e  `burst-iops` opzioni con il  `qos-create` comando e il  `qos-modify` comando per modificare queste impostazioni predefinite. In ONTAP 9.18.1 e versioni precedenti, le impostazioni predefinite non possono essere modificate.
...
|===
| Supporto dei carichi di lavoro | ONTAP 9.8 e versioni successive | ONTAP da 9,7 a 9,4 | ONTAP 9.3 e versioni precedenti 
...
|===
^1^ A partire da ONTAP 9.9.1, l'accesso SMB è supportato anche nei qtree nei volumi FlexVol e FlexGroup con SMB abilitato.  A partire da ONTAP 9.8, l'accesso NFS è supportato nei qtree nei volumi FlexVol e FlexGroup con NFS abilitato.
...
|===
| Supporto dei carichi di lavoro | ONTAP 9.13.1 e versioni successive | ONTAP da 9,8 a 9.13.0 | ONTAP da 9,4 a 9,7 | ONTAP 9.3 
...
| Volume FlexGroup | sì | sì | sì | no 


| qtree ^1^ | sì | sì | no | no 


| Carichi di lavoro multipli per gruppo di policy | sì | sì | sì | no 


| Gruppi di criteri non condivisi | sì | sì | sì | no 
|===
...
|===
| Supporto dei carichi di lavoro | ONTAP 9.13.1 e versioni successive | ONTAP da 9,4 a 9.13.0 | ONTAP 9.3 
...
|===
| Supporto dei carichi di lavoro | ONTAP 9.4 e versioni successive | ONTAP 9.3 e versioni precedenti 
