...
+
`network fcp adapter modify -node _nodename_ -adapter _adapter_ -status-admin _down_`
...
+
`network fcp adapter modify -node _nodename_ -adapter _adapter_ -status-admin up`
