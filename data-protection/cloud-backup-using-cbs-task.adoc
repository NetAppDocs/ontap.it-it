...
NOTE: A partire da ONTAP 9.10.1, puoi selezionare StorageGRID come provider di backup cloud, ma solo se BlueXP  viene implementato on-premise. BlueXP Connector deve essere installato on-premise e disponibile tramite l'applicazione Software-as-a-Service (SaaS) BlueXP.
