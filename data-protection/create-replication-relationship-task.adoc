...
NOTE: Il `schedule` Il parametro non è applicabile quando si creano relazioni sincroni di SnapMirror.
...
NOTE: Il criterio predefinito `SnapCenterSync` è di tipo `Sync`. Questo criterio replica qualsiasi copia Snapshot creata con `snapmirror-label` di "app_coerente".
