...
----
. Dalla SVM di origine originale o dal cluster di origine originale, rilasciare la relazione originale mantenendo intatte le copie Snapshot:
+
...
----
. Dalla SVM di destinazione originale o dal cluster di destinazione originale, rilasciare la relazione invertita mantenendo intatte le copie Snapshot:
+
`snapmirror release -source-path SVM: -destination-path SVM: -relationship-info-only true`
