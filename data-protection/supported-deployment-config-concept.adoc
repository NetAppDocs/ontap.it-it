...
Le implementazioni fan-out e con cascate supportano qualsiasi combinazione di DR SnapMirror, SnapVault o replica unificata. Tuttavia, le relazioni sincrone SnapMirror (supportate a partire da ONTAP 9.5) supportano solo implementazioni fan-out con una o più relazioni asincrone SnapMirror e non supportano le implementazioni con cascate. Solo una relazione nella configurazione fan-out può essere una relazione sincrona SnapMirror, tutte le altre relazioni del volume di origine devono essere relazioni asincrone SnapMirror. xref:../snapmirror-active-sync/recover-unplanned-failover-task.html[Sincronizzazione attiva di SnapMirror] (Supportato a partire da ONTAP 9.3,1) supporta anche configurazioni fan-out.
