...
[role="lead"]
A partire da ONTAP 9.9.1, è possibile utilizzare SnapMirror cloud per migrare i flussi di lavoro di protezione dei dati sul cloud. Invece delle tradizionali architetture di backup su nastro, è possibile utilizzare storage a oggetti come repository per la conservazione e l'archiviazione dei dati a lungo termine. È possibile utilizzare provider come Amazon S3, Azure Blob, Google Cloud Storage, StorageGRID o ONTAP S3 come archivio di oggetti.
