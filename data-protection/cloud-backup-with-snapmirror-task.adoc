...
---
= Eseguire il backup dei dati sul cloud utilizzando ONTAP SnapMirror cloud
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
_SnapMirror cloud_ offre la replica ONTAP verso destinazioni archivio di oggetti. Questo ti permette di sostituire le architetture di backup su nastro tradizionali con storage a oggetti per la conservazione e l'archiviazione dei dati a lungo termine. Puoi usare provider come Amazon S3, Alibaba Cloud, Azure Blob, Google Cloud Storage, IBM Cloud Object Storage, NetApp StorageGRID o ONTAP S3 come tuo archivio di oggetti cloud.


NOTE: La replica cloud di SnapMirror gestita solo da SystemManager supporta StorageGrid e ONTAP S3 come destinazione dello storage a oggetti. Altri provider non sono supportati.
...
* SnapMirror cloud aumenta il numero di bucket di storage a oggetti S3 supportati per cluster da 6 a 100. Questo limite si applica al numero totale di archivi di oggetti configurati su tutti gli endpoint di SnapMirror cloud object store in un cluster ONTAP. Quando si utilizza questa funzionalità, tenere presenti le seguenti considerazioni:
+
...
* SnapMirror cloud supporta le operazioni di backup e ripristino per i volumi FlexGroup su configurazioni MetroCluster utilizzando l'esistente link:https://docs.netapp.com/us-en/ontap-automation/get-started/access_rest_api.html["API REST di ONTAP"^]. Questa funzionalità consente di creare relazioni SnapMirror cloud per i volumi FlexGroup su configurazioni MetroCluster che sono gestite dal sito partner dopo uno switchover e uno switchback.


Prima di utilizzare la funzionalità cloud di SnapMirror, dovresti richiedere una chiave di licenza API SnapMirror cloud dal sito di supporto NetApp: link:https://mysupport.netapp.com/site/tools/snapmirror-cloud-api-key["Richiedere la chiave di licenza dell'API cloud SnapMirror"^].
