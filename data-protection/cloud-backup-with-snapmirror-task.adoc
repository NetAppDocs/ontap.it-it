...
[role="lead"]
A partire da ONTAP 9.9.1, è possibile utilizzare  quando si desidera migrare i flussi di lavoro di protezione dei dati sul cloud. Invece delle tradizionali architetture di backup su nastro, è possibile utilizzare storage a oggetti come repository alternativo per la conservazione e l'archiviazione dei dati a lungo termine. È possibile utilizzare provider come Amazon S3, Alibaba Cloud, Azure Blob, Google Cloud Storage, IBM Cloud Object Storage, StorageGRID o ONTAP S3 come archivio di oggetti cloud.

A partire da ONTAP 9.9.1, è possibile utilizzare System Manager per eseguire il backup e il ripristino dei dati.
