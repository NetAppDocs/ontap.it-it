...
| Replica Snapshot creata dall'applicazione | ONTAP 9.7 | Se uno snapshot viene contrassegnato con l'etichetta appropriata al momento dell' `snapshot create`operazione, utilizzando la CLI o l'API ONTAP, SnapMirror sincrono replica gli snapshot, sia quelli creati dall'utente sia quelli creati con script esterni, dopo aver messo in quiescenza le applicazioni. A partire da ONTAP 9.14.1, il requisito dell'etichetta per la replica degli snapshot creati dalle applicazioni non esiste più. Per ulteriori informazioni sulla replica degli snapshot creati dalle applicazioni, consultare la link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["Knowledge Base NetApp : come replicare gli snapshot creati dall'applicazione con SnapMirror sincrono"^]. 


| Protezione autonoma dal ransomware | ONTAP 9.19.1 |  
...
| Ripristino NDMP/NDMP | ONTAP 9.13.1 | Sia il cluster di origine che il cluster di destinazione devono eseguire ONTAP 9.13.1 o versioni successive per poter effettuare backup NDMP basati su dump o SMTape dei volumi sincroni SnapMirror. I ripristini NDMP, sia basati su dump che su SMTape, non sono supportati verso volumi di destinazione in una relazione SnapMirror active sync o SnapMirror synchronous. 
...
| Replica delle operazioni di gestione degli snapshot | ONTAP 9.19.1 | A partire da ONTAP 9.19.1, le operazioni di gestione degli snapshot `snapshot delete`, `snapshot rename`, e `snapshot modify` sono replicate nel cluster secondario. 


| Replica snapshot pianificata | ONTAP 9.19.1 | A partire da ONTAP 9.19.1, è possibile abilitare la replica degli snapshot pianificati tramite una SnapMirror policy. Nelle versioni precedenti a ONTAP 9.19.1, gli snapshot pianificati creati utilizzando una policy di snapshot non vengono replicati. 
...
| Disaster recovery SVM | ONTAP 9.6  a| 
* Una SnapMirror sincrona può anche essere una sorgente di ripristino di emergenza SVM, ad esempio, una configurazione fan-out con SnapMirror sincrona come una delle due e il ripristino di emergenza SVM come l'altra.
* Una sorgente SnapMirror sincrona non può essere una destinazione di disaster recovery SVM perché SnapMirror synchronous non supporta il cascading di una sorgente di protezione dei dati. Devi rilasciare la relazione sincrona prima di eseguire una risincronizzazione flip di disaster recovery SVM nel cluster di destinazione.
* Una destinazione SnapMirror sincrona non può essere una sorgente di ripristino di emergenza SVM perché il ripristino di emergenza SVM non supporta la replica dei volumi DP. Una risincronizzazione inversa della sorgente sincrona comporterebbe che il ripristino di emergenza SVM escluda il volume DP nel cluster di destinazione.




| link:../snaplock/snapshot-lock-concept.html["Snapshot a prova di manomissione"] | ONTAP 9.19.1 | È possibile specificare la data di scadenza solo durante la creazione dello snapshot. Non è possibile impostare o estendere la data di scadenza degli snapshot esistenti. 
...
* SnapCenter
* Volumi SnapLock
* Ripristino NDMP basato su Dump o SMTape su volumi di destinazione in una relazione di sincronizzazione attiva SnapMirror o sincrona SnapMirror
...
== Replica snapshot pianificata

A partire da ONTAP 9.19.1, è possibile utilizzare il parametro della policy SnapMirror `-sync-snapshot-replication-mode` in modalità privilegio avanzato per replicare le snapshot pianificate nella destinazione di SnapMirror. La replica delle snapshot pianificate fornisce simmetria tra i volumi primari e secondari in una relazione sincrona SnapsMirror.

.Prerequisiti
* Devi essere un amministratore del cluster o di SVM
* Devi essere in modalità privilegi avanzato
* È necessario configurare una policy SnapMirror definita dall'utente che utilizzi il parametro  `-sync-snapshot-replication-mode` e includa l'opzione  `sched_created`.
+
** Le policy definite dal sistema non possono essere modificate. È necessario creare una nuova policy e ristabilire la relazione.
** Le modifiche alle policy definite dall'utente richiedono una risincronizzazione (sospensione seguita da ripresa) per avere effetto sulle relazioni esistenti.




Il parametro accetta le seguenti opzioni:

* `none`- disabilita la replica degli snapshot SnapMirror non sincroni
* `application_created`- replica gli snapshot creati dall'applicazione
* `sched_created` - replica gli snapshot programmati
* `sm_created` - replica gli snapshot creati da SnapMirror asincrono (fanout leg)
* Un elenco separato da virgole di tipi: `application_created`, `sched_created` e `sm_created`.


Il parametro ha effetto solo quando lo stato della relazione è `InSync`. I tipi di Snapshot non elencati vengono memorizzati solo sul volume primario.

A partire da ONTAP 9.19.1, `snapmirror initialize` replica tutti gli snapshot nel cluster secondario, mentre `snapmirror resync` replica solo i nuovi snapshot creati dopo l'ultimo snapshot comune. Durante un'operazione di risincronizzazione, gli snapshot non replicati contrassegnati come primary-only quando lo stato della relazione è `InSync` non vengono replicati nel cluster secondario, e tali snapshot vengono mantenuti nel nuovo cluster secondario dopo un'operazione di risincronizzazione invertita.


NOTE: La creazione di snapshot su volumi sincroni SnapMirror può aumentare la latenza di I/O utente. Si consiglia di utilizzare pianificazioni di snapshot con una frequenza superiore a 30 minuti.

.Informazioni correlate
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["Report tecnico NetApp 4733: Configurazione sincrona e Best practice SnapMirror"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["volume snapshot create"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["eliminazione snapshot del volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["modifica snapshot volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["rinomina snapshot volume"^]
