...
* A partire da ONTAP 9.19.1, l'abilitazione di IPsec non è supportata sugli SVM configurati per SVM DR. Se usi IPsec, devi disabilitarlo oppure scegliere un SVM diverso per configurare una relazione SVM DR. Riceverai un errore se usi ONTAP 9.19.1 o versioni successive e provi ad abilitare IPsec su un SVM configurato per SVM DR.
...
. Dal cluster di origine, crea una relazione peer SVM utilizzando il comando  `vserver peer create`.
+
