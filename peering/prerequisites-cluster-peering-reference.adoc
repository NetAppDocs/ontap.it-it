...
[role="lead"]
Prima di configurare il peering del cluster, è necessario verificare che siano soddisfatti i requisiti di connettività, porta, indirizzo IP, subnet, firewall e cluster.
...
== Limiti peer del cluster

Il numero di cluster peer supportati dipende dalla versione di ONTAP:

* ONTAP 9.14.1 e versioni successive: sono supportati fino a 400 cluster peer.
* Versioni precedenti di ONTAP: sono supportati fino a 255 peer del cluster.
+
....
[NOTE]
