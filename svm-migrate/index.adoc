...
| SVM MetroCluster | ONTAP 9.16.1 | Puoi migrare una SVM da una coppia ha non MetroCluster in una configurazione MetroCluster o da una configurazione MetroCluster a una coppia ha non MetroCluster. Non è possibile migrare una SVM da una configurazione MetroCluster a un'altra configurazione MetroCluster. [NOTA] la migrazione delle SVM non supporta la migrazione delle SVM MetroCluster nelle release precedenti a ONTAP 9.16,1. Potrebbe essere possibile utilizzare la replica asincrona di SnapMirror in link:https://www.netapp.com/media/83785-tr-4966.pdf["Migrare una SVM in una configurazione MetroCluster"^]. Devi essere consapevole del fatto che l'utilizzo di SnapMirror Asynchronous per la migrazione di una SVM in una configurazione MetroCluster è un metodo di migrazione _dirompente_. 
