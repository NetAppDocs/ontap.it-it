...
NOTE: Durante la migrazione da un cluster AFF a un cluster FAS con aggregati ibridi, il posizionamento automatico del volume tenterà di eseguire una corrispondenza di aggregati simili.  Ad esempio, se il cluster di origine ha 60 volumi, il posizionamento del volume tenterà di trovare un aggregato AFF sulla destinazione in cui posizionare i volumi.  Se non c'è spazio sufficiente sugli aggregati AFF , i volumi verranno posizionati su aggregati con dischi non flash.
