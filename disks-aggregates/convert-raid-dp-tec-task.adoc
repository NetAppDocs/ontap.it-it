...
.Processo per flashpools/aggregati ibridi in cui è necessario utilizzare -disktype per modificare ciascun tipo di disco
[-T, -disktype {ATA | BSAS | FCAL | FSAS | LUN | MSATA | SAS | SSD | VMDISK | SSD-NVM | SSD-CAP | SSD-ZNS | VMLUN | VMLUN-SSD}] - Tipo di disco

Questo parametro specifica il tipo di disco dei gruppi RAID da modificare. Nel caso di Flash Pool, specifica il livello HDD o il livello SSD. Se il livello HDD è composto da più di un tipo di disco, la specifica di uno qualsiasi dei tipi di disco in uso determina la modifica di quel livello. Se il tipo di RAID aggregato corrente è mixed_raid_type, questo parametro è obbligatorio.
