...
NOTE: È inoltre possibile salvare le specifiche di questi LUN in un Ansible Playbook. Per maggiori dettagli, consultare link:task_admin_use_ansible_playbooks_add_edit_volumes_luns.html["Utilizza i Playbook Ansible per aggiungere o modificare volumi o LUN"].
