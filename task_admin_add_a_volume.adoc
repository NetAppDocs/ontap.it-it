...
---
= Aggiungi un volume con ONTAP System Manager
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
+
** *Esporta tramite NFS*: crea il volume con la  `default` policy di esportazione che garantisce agli utenti l'accesso completo ai dati.
...
NOTE: È inoltre possibile salvare le specifiche di questo volume in un Ansible Playbook. Per maggiori dettagli, consultare link:task_use_ansible_playbooks_add_edit_volumes_luns.html["Utilizza i Playbook Ansible per aggiungere o modificare volumi o LUN"].
