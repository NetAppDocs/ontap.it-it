...
+
A partire da ONTAP 9.9.1, le relazioni SnapMirror fanout sono supportate per i volumi FlexGroup. Per ulteriori informazioni, vedere link:https://docs.netapp.com/us-en/ontap/flexgroup/create-snapmirror-cascade-fanout-reference.html#considerations-for-creating-cascading-relationships["Considerazioni per la creazione di relazioni a cascata e fan-out di SnapMirror per FlexGroups"].
...
.. Verificare il processo di conversione: `volume conversion start -vserver vs1 -volume src_vol -check-only true`
+
...
+
.. Interrompere la relazione di SnapMirror di FlexVol: `snapmirror quiesce -destination-path dest_svm:dest_volume`
+
[listing]
----
cluster2::> snapmirror quiesce -destination-path vs2:dst_xdp
----
.. Avviare la conversione: `volume conversion start -vserver dest_svm -volume dest_volume`
+
[listing]
----
...
. Convertire il volume FlexVol di origine in volume FlexGroup: `volume conversion start -vserver src_svm_name -volume src_vol_name`
+
[listing]
----
...
----
. Risincronizzare la relazione: `snapmirror resync -destination-path dest_svm_name:dest_volume`
+
[listing]
----
