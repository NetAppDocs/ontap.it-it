...
I diritti vengono concessi per volume.

È possibile spostare solo le directory.  Non è possibile spostare i file nella directory .ontaptrashbin.

link:fast-directory-delete-asynchronous-task.html#delete-directories-asynchronously["Scopri come usare ONTAP per eliminare directory in modo asincrono dai volumi FlexVol e FlexGroup"].
...
.Fasi
. Dalla cluster CLI, accedere alla modalità con privilegi avanzati: `-privilege advanced`
