...
.. Definire una regola per il criterio SnapVault che determina quali copie Snapshot vengono trasferite durante le operazioni di inizializzazione e aggiornamento: `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
