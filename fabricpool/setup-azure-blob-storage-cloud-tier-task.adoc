...
+
I nuovi sistemi AFF ordinati sono dotati di 10 TB di capacità libera per l'utilizzo di FabricPool. Se hai bisogno di capacità aggiuntiva in un sistema AFF, se utilizzi l'archiviazione BLOB di Azure in un sistema non AFF o se esegui l'upgrade da un cluster esistente, hai bisogno di un link:../fabricpool/install-license-aws-azure-ibm-task.html["Licenza FabricPool"].
