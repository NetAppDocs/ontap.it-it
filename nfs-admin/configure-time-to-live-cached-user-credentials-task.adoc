...
summary: 'È possibile configurare la durata per cui ONTAP memorizza le credenziali degli utenti NFS nella sua cache interna modificando il server NFS della storage virtual machine. Ciò consente di alleviare alcuni problemi relativi a un carico elevato sui name server o a modifiche delle credenziali che influiscono sull"accesso degli utenti NFS.' 
---
...
Il TTL si misura in millisecondi. A partire da ONTAP 9.17.1 e versioni successive, il valore predefinito è 1 ora (3,600,000 millisecondi). In ONTAP 9.16.1 e versioni precedenti, il valore predefinito è 24 ore (86,400,000 millisecondi). L'intervallo consentito per questo valore va da 1 minuto (60,000 millisecondi) a 7 giorni (604,800,000 millisecondi).
