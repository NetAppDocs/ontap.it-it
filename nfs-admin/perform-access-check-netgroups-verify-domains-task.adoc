...
summary: 'Per impostazione predefinita, ONTAP esegue una verifica aggiuntiva durante la valutazione dell"accesso di un client a un netgroup. Questo controllo aggiuntivo garantisce che il dominio del client corrisponda alla configurazione di dominio della storage virtual machine. In caso contrario, ONTAP nega l"accesso al client.' 
---
