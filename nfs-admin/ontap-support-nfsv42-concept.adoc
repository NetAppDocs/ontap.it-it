...
NFSv4.2 è abilitato per impostazione predefinita in ONTAP 9.9.1 e versioni successive.  In ONTAP 9.8, è necessario abilitare manualmente v4.2 specificando `-v4.2` opzione e impostandola su `enabled` durante la creazione di un server NFS sulla macchina virtuale di archiviazione (SVM).  L'abilitazione di NFSv4.1 consente inoltre ai client di utilizzare le funzionalità di NFSv4.1 mentre sono montati come v4.2.
