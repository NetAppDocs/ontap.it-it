...
. Selezionare image:icon_kabob.gif["Opzioni di menu"] E scegliere *Modifica > Volume*.
...
. Individuare *Snapshot Policies* e selezionare image:icon_arrow.gif["freccia"].
. Nella finestra *Add Snapshot Policy*, inserire il nome del criterio.
. Selezionare image:icon_add.gif["Aggiungi"].
...
. Selezionare *Storage > Volumes* (archiviazione > volumi) e selezionare un volume.
. Nella pagina dei dettagli del volume, selezionare la scheda *copie Snapshot*.
. Selezionare la copia Snapshot, quindi image:icon_kabob.gif["Opzioni di menu"]E scegliere *Modify SnapLock Expiration Time* (Modifica ora di scadenza protocollo). È possibile selezionare il calendario per scegliere la data e l'ora di scadenza della conservazione.
. Salvare le modifiche.
. Nella pagina *volumi > copie Snapshot*, selezionare *Mostra/Nascondi* e scegliere *ora scadenza SnapLock* per visualizzare la colonna *ora scadenza SnapLock* e verificare che il tempo di conservazione sia impostato.
