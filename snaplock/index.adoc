...
Con SnapLock, esegui il commit di file e snapshot nello storage WORM e imposti i periodi di conservazione per i dati protetti da WORM. Lo storage WORM SnapLock utilizza la tecnologia snapshot NetApp e può sfruttare la replica SnapMirror e i backup SnapVault come tecnologia di base per offrire protezione e ripristino dei dati. Scopri di più sullo storage WORM: link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["Storage WORM conforme con NetApp SnapLock - TR-4526"^].
...
SnapLock è una funzionalità di ONTAP basata su licenza. Una singola licenza ti dà diritto a utilizzare SnapLock in modalità Compliance rigorosa, per soddisfare i mandati esterni come la norma SEC 17a-4(f), e in modalità Enterprise più flessibile, per soddisfare le normative interne obbligatorie per la protezione delle risorse digitali. Le licenze SnapLock fanno parte del link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["ONTAP uno"] suite software.
...
A livello di file
 a| 
Può essere eliminato dall'amministratore della conformità utilizzando una procedura di "eliminazione privilegiata" verificata
