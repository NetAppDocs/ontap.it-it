...
* È possibile utilizzare SnapLock per SnapVault per proteggere WORM le copie Snapshot sullo storage secondario. Vedere link:commit-snapshot-copies-worm-concept.html["Assegnare le copie Snapshot a WORM"].
...
SnapLock è una funzionalità basata su licenza di NetApp ONTAP. Una singola licenza consente di utilizzare SnapLock in modalità di conformità rigorosa, per soddisfare mandati esterni come la norma SEC 17a-4 e una modalità aziendale più allentata, per soddisfare le normative interne per la protezione delle risorse digitali. Le licenze SnapLock fanno parte di link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["ONTAP uno"] suite software.
...
* link:commit-snapshot-copies-worm-concept.html["Assegnare copie Snapshot a WORM per lo storage secondario"]
...
* link:snapshot-lock-concept.html["Bloccare una copia Snapshot per la protezione dagli attacchi ransomware"]
* link:create-audit-log-task.html["Esaminare l'utilizzo di SnapLock con il registro di controllo"]
