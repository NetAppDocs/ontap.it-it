...
---
= Imposta il tempo di conservazione di ONTAP SnapLock
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP impedisce di ripristinare un cluster da ONTAP 9.10.1 a una versione ONTAP precedente quando sono presenti file con un periodo di conservazione successivo a "19 gennaio 2071 8:44:07 AM".
...
IMPORTANT: Nelle versioni precedenti a ONTAP 9.10.1, se non si imposta esplicitamente il tempo di conservazione prima di eseguire il commit di un file in modalità Compliance nello stato WORM e non si modificano i valori predefiniti, il file verrà conservato per 30 anni. Questa modifica _non_ può essere annullata. Analogamente, in ONTAP 9.10.1 e versioni successive, se non si imposta esplicitamente il tempo di conservazione prima di eseguire il commit di un file in modalità Enterprise nello stato WORM e non si modificano i valori predefiniti, il file verrà conservato per 0 anni o, di fatto, per niente.
...
====
Per un utilizzo avanzato, vedi link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["Storage WORM conforme con NetApp SnapLock"^].
