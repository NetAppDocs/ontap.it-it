...
. Assicurarsi che il certificato CA (insieme a qualsiasi autorità di firma intermedia fino alla CA radice) utilizzato per firmare il certificato per il server NetApp sia nel formato Privacy Enhanced Mail (PEM) e denominato "Ontap_CA.pem". Posizionarlo nella directory di installazione di Antivirus Connector. Sul sistema ONTAP , installare il certificato CA (insieme a qualsiasi autorità di firma intermedia fino alla CA radice) utilizzato per firmare il certificato client per Antivirus Connector su "ONTAP" come certificato di tipo "client-ca".
