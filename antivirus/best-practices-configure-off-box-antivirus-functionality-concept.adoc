...
* Il  `scan-queue-timeout` valore deve essere inferiore al  `scanner-pool request-timeout` valore. Se è impostato su un valore superiore, l'accesso ai file potrebbe essere ritardato e potrebbe infine andare in timeout. Per evitare questo, configura il  `scan-queue-timeout` a 5 secondi in meno rispetto al  `scanner-pool request-timeout` valore. Consulta la documentazione del fornitore del motore di scansione per istruzioni su come modificare le impostazioni  `scan-queue-timeout`. Il  `scanner-pool timeout` può essere cambiato usando il seguente comando in modalità avanzata e fornendo il valore appropriato per il parametro  `request-timeout`:
