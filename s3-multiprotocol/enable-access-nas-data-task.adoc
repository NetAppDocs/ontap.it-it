...
+
.. Fai clic su *Archiviazione > Macchine virtuali di archiviazione*, seleziona una macchina virtuale di archiviazione compatibile con NAS, fai clic su *Impostazioni* e quindi in *Protocolli > S3* fai clic su image:icon_gear.gif["Icona delle azioni"] S3.
...
. Abilitare S3 su una VM di storage con protocolli NAS configurati.
+
.. Fai clic su *Cluster > Storage VMs*, seleziona una Storage VM pronta per NAS, fai clic su *Impostazioni* e quindi in *Protocolli > S3* fai clic su image:icon_gear.gif["Icona delle azioni"].
.. Selezionare il tipo di certificato. Se si seleziona un certificato generato dal sistema o uno dei propri, questo sarà necessario per l'accesso del client.
.. Inserire le interfacce di rete.
...
+
** La chiave segreta non viene visualizzata di nuovo.
** Se hai bisogno nuovamente delle informazioni sul certificato: fai clic su *Cluster > Storage VM*, seleziona la Storage VM e fai clic su *Impostazioni*.
