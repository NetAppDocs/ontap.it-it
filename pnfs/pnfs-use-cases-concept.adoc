...
summary: È possibile utilizzare pNFS con diverse funzionalità ONTAP, tra cui nconnect, session trunking e bilanciamento avanzato della capacità, per migliorare le prestazioni e la flessibilità. 
---
...
[role="lead"]
È possibile utilizzare pNFS con diverse funzionalità ONTAP per migliorare le prestazioni e offrire maggiore flessibilità per i carichi di lavoro NFS.
...
In ONTAP, il trunking di sessione NFSv4.1 viene utilizzato su più interfacce fisiche su un singolo nodo in un cluster per fornire throughput aggiuntivo e ridondanza su connessioni indipendenti.

image::pNFS_fig_006.png[Session trunking NFSv4.1 su più interfacce su un singolo nodo]

Se utilizzati con pNFS, questi trunk possono fornire prestazioni aggregate complessive superiori sull'intero cluster.

image::pNFS_fig_021.png[NFSv4.1 con prestazioni pNFS attraverso il cluster]

Il trunking di sessione viene stabilito in diversi modi:

* *Rilevamento automatico tramite opzioni di montaggio:* Nella maggior parte dei client NFS moderni, il trunking di sessione viene stabilito utilizzando opzioni di montaggio (consultare la documentazione del fornitore del sistema operativo) che segnalano al server NFS di inviare informazioni al client sui trunk di sessione. Queste informazioni appaiono tramite un pacchetto NFS come una  `fs_location4` chiamata.
+
...
Quando  `trunkdiscovery` viene utilizzato, viene sfruttata una chiamata GETATTR aggiuntiva (FS_Locations) per le interfacce trunk di sessione elencate sul nodo server NFS in cui si trova l'interfaccia di montaggio. Una volta che queste vengono restituite, i montaggi successivi vengono effettuati agli indirizzi restituiti. Questo si osserva in una cattura di pacchetti durante il montaggio.
...
È possibile utilizzare i referral NFSv4.1 con pNFS, ma la funzionalità è inutile. Abilitare i referral con pNFS non mostrerà risultati evidenti.
