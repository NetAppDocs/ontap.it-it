...
* Si desidera accedere a un sottoinsieme specifico di dati di una LUN (un volume logico o file system in un gruppo di volumi, oppure un file o un insieme di file specifico in un file system) e copiarlo nella LUN originale, senza ripristinare il resto dei dati nella LUN originale. Questo funziona sui sistemi operativi che supportano il montaggio di una LUN e di un clone della LUN allo stesso tempo.
