...
summary: 'Microsoft Offloaded Data Transfer, noto anche come offload di copia, consente trasferimenti diretti di dati all"interno di un dispositivo di storage o tra dispositivi di storage compatibili senza trasferire i dati attraverso il computer host.' 
---
...
Quando si utilizza VMware e Microsoft Copy Offload in ambienti virtualizzati, è necessario allineare le LUN.  Le LUN non allineate possono compromettere le prestazioni. link:io-misalignments-properly-aligned-luns-concept.html["Scopri di più sui LUN non allineati"].
