...
summary: È possibile ripristinare una singola LUN in una nuova posizione o nella posizione originale. È possibile eseguire il ripristino da qualsiasi copia Snapshot nel volume secondario SnapVault. Per ripristinare il LUN nella posizione originale, ripristinarlo in una nuova posizione, quindi copiarlo. 
---
...
vserverA:srcvolA
         XDP  vserverB:dstvolB
                        Snapmirrored
                                Idle       -         true    -
----
. Eseguire `volume snapshot show` Per identificare la copia Snapshot da cui si desidera ripristinare il LUN.
+
[listing]
----
...
----
. Eseguire `volume clone create` Per creare un clone di lettura/scrittura dalla copia Snapshot desiderata.
+
