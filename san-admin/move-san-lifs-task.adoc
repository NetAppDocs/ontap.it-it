...
summary: Se un nodo deve essere messo offline, è possibile spostare un SAN LIF per preservarne le informazioni di configurazione, come il suo WWPN, ed evitare di riconfigurare la switch fabric. Poiché un SAN LIF deve essere messo offline prima di essere spostato, il traffico host deve utilizzare un software di multipathing host per garantire un accesso continuo al LUN. È possibile spostare i SAN LIF su qualsiasi nodo di un cluster, ma non è possibile spostare i SAN LIF tra storage virtual machine. 
---
