...
[source, cli]
----
network fcp adapter modify -node _node_name_ -adapter _adapter_port_ -status-admin up
----
...
. Se disponi di un sistema AFF, ASA o FAS, segui questi passaggi per creare un LIF: Se disponi di un sistema ASA r2, segui link:https://docs.netapp.com/us-en/asa-r2/administer/manage-client-vm-access.html#create-a-lif-network-interface["questa procedura di System Manager"^] per creare un LIF.
+
...
[source, cli]
----
network fcp adapter modify -node node1 -adapter 0c -status-admin down
----
+
Se l'adattatore non viene scollegato, è anche possibile rimuovere il cavo dalla porta dell'adattatore appropriata sul sistema.
...
[source, cli]
----
network fcp adapter modify -node node1 -adapter 0c -status-admin up
----
