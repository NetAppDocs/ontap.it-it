...
.Prima di iniziare
È necessario disporre di una licenza FC (link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["Incluso con ONTAP One"]) e deve essere attivata. Se la licenza FC non è abilitata, le LIF e le SVM sembrano essere online, ma lo stato operativo è `down`. Il servizio FC deve essere abilitato affinché i tuoi LIF e SVM siano operativi. Per ospitare gli iniziatori, è necessario utilizzare lo zoning initiator singolo per tutte le LIF FC nella SVM.
