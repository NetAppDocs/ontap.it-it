...
[role="lead"]
Quando si configura lo storage, si abilita l'accesso in lettura e scrittura agli ONTAP storage systems da parte degli host NVMe. Per i sistemi che utilizzano il protocollo NVMe, è necessario creare uno o più namespace e sottosistemi NVMe. Ciascun namespace può quindi essere mappato a un sottosistema NVMe per consentire l'accesso ai dati dall'host.

Se la macchina virtuale di storage (SVM) contenente il sottosistema NVMe dispone di interfacce di rete sia NVMe/FC che NVMe/TCP, è possibile accedere agli spazi dei nomi mappati al sottosistema NVMe tramite entrambi i protocolli di trasporto NVMe/FC e NVMe/TCP. Il supporto NVMe per FC è disponibile a partire da ONTAP 9.4 e versioni successive. Il supporto NVMe per TCP è disponibile a partire da ONTAP 9.10.1 e versioni successive. Se è necessario un accesso specifico per protocollo agli spazi dei nomi, è necessario link:configure-svm-nvme-task.html["configurare una SVM"] per ciascun protocollo.
