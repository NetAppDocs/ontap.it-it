...
[role="lead"]
Se non è disponibile una procedura per l'host specifico, è possibile utilizzare questa procedura per creare spazi dei nomi e fornire lo storage per qualsiasi host supportato da NVMe.

Questa procedura crea nuovi spazi dei nomi su una VM di storage esistente. La VM di storage deve essere configurata per NVME e il trasporto FC o TCP deve essere già impostato.
