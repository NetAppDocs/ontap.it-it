...
summary: ONTAP potrebbe segnalare disallineamenti di input e output su LUN correttamente allineati. In generale, questi avvisi di disallineamento possono essere ignorati a condizione che si sia certi che la LUN sia stata configurata correttamente e che la tabella delle partizioni sia corretta. 
---
...
Se hai bisogno di ulteriori indagini, consulta illink:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_to_identify_unaligned_IO_on_LUNs["Knowledge Base NetApp : come identificare gli IO non allineati sulle LUN?"^]
...
Per ulteriori informazioni sui LUN forniti in ONTAP 9.7 o versioni precedenti, vederelink:https://kb.netapp.com/onprem/ontap/da/SAN/How_to_identify_unaligned_IO_on_LUNs["Knowledge Base NetApp : come identificare gli IO non allineati sulle LUN"^] .
