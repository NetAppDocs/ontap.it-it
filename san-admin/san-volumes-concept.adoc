...
summary: 'ONTAP offre tre opzioni di base per il provisioning dei volumi: thick provisioning, thin provisioning e semi-thick provisioning. Ciascuna opzione utilizza metodi diversi per gestire lo spazio dei volumi e i requisiti di spazio per le tecnologie di condivisione dei blocchi di ONTAP. Comprendere il funzionamento di queste opzioni consente di scegliere quella più adatta al proprio ambiente.' 
---
