...
| Visualizzazione in System Manager | X | X | X | X | X | X 


| Analisi della capacità | X | X | X | X | X | X 


| Informazioni sui dati inattivi | X | X | X | X | X | X 


| Supporto per volumi in transizione da Data ONTAP 7-Mode | X | X | X | X | X |  


| Possibilità di personalizzare il periodo inattivo in System Manager | X | X | X | X | X |  


| Monitoraggio delle attività a livello di volume | X | X | X | X |  |  


| Scarica i dati di Activity Tracking in CSV | X | X | X | X |  |  


| Monitoraggio delle attività a livello di SVM | X | X | X |  |  |  


| Tempistiche | X | X | X |  |  |  


| Analisi dell'utilizzo | X | X |  |  |  |  


| Opzione per attivare l'analisi del file system per impostazione predefinita | X |  |  |  |  |  
|===
