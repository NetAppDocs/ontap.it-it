...
summary: 'È possibile gestire il messaggio del giorno a livello di cluster o di SVM. Per impostazione predefinita, il messaggio del giorno (MOTD) configurato per il cluster è abilitato anche per tutte le SVM. Inoltre, è possibile configurare un MOTD a livello di SVM per ciascuna SVM. Il MOTD a livello di cluster può essere abilitato o disabilitato per ogni SVM dall"amministratore del cluster.' 
---
