...
È possibile accedere alle opzioni di Nodeshell supportate in ClusterShell utilizzando `vserver options clustershell` comando. Per visualizzare queste opzioni, è possibile interrogare la CLI di Clustershell con `vserver options -vserver _nodename_or_clustername_ -option-name ?`
