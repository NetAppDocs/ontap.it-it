...
.Di cosa hai bisogno
* Se si aggiungono nodi a un cluster a più nodi, più della metà dei nodi esistenti nel cluster deve essere integro (indicato da `cluster show`).
* Se si aggiungono nodi a un cluster senza switch a due nodi, è necessario aver installato e configurato gli switch di interconnessione e gestione del cluster prima di aggiungere altri nodi.
+
La funzionalità cluster senza switch è supportata solo in un cluster a due nodi.

+
Quando un cluster contiene o aumenta fino a più di due nodi, l'ha del cluster non è richiesto e viene disattivato automaticamente.
...
* Se nel cluster è attivata la configurazione automatica SP, la subnet specificata per l'SP da utilizzare deve disporre di risorse disponibili per il nodo di giunzione.
+
Un nodo che si unisce al cluster utilizza la subnet specificata per eseguire la configurazione automatica dell'SP.
