...
È necessario solo link:task_create_the_cluster_on_the_first_node.html["USA l'interfaccia a riga di comando di ONTAP per configurare il cluster"]se si esegue ONTAP 9.7 o versioni precedenti su una configurazione MetroCluster.

A partire da ONTAP 9.13.1, sulle piattaforme AFF A800 e FAS8700, puoi anche utilizzare l'interfaccia a riga di comando di ONTAP per creare e configurare nuovi cluster in ambienti di rete solo IPv6. Se è necessario utilizzare IPv6 in ONTAP 9.13,0 e versioni precedenti o su altre piattaforme in ONTAP 9.13,1 e versioni successive, è possibile utilizzare Gestione sistema per creare nuovi cluster utilizzando IPv4 e versioni successivelink:convert-ipv4-to-ipv6-task.html["Converti in IPv6"].
